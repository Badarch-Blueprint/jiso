...
pick_port() {
	# Use process.stdout.write(String(p)) rather than console.log(p): console.log applies
	# util.inspect formatting to a numeric arg, which ANSI-colorizes it when color is forced
	# (e.g. FORCE_COLOR=3 in the environment). That ESC pollution then corrupts the JSON the
	# launcher emits. Writing a plain string is never colorized.
	node -e '
...
# immediately. If code.sh dies or we time out, dump the log so the failure is visible.
#
# IMPORTANT: Chromium does NOT guarantee it binds the requested --remote-debugging-port.
# If that port is unavailable (e.g. a TIME_WAIT/leftover socket from a prior instance), it
# silently picks a different free port. It ALWAYS records the actual port (and ws path) in
# <user-data-dir>/DevToolsActivePort. So we resolve the real port from that file rather than
# polling the requested $CDP_PORT (which would time out even though CDP is up on another port).
echo "[launch.sh] waiting for CDP (requested $CDP_PORT; resolving actual port via DevToolsActivePort)..." >&2
DEVTOOLS_PORT_FILE="$DEST_UDD/DevToolsActivePort"
READY=0
for i in $(seq 1 90); do
	...
	if [[ -f "$DEVTOOLS_PORT_FILE" ]]; then
		ACTUAL_CDP_PORT=$(head -n1 "$DEVTOOLS_PORT_FILE" 2>/dev/null | tr -dc '0-9')
		if [[ -n "$ACTUAL_CDP_PORT" ]] && curl -sf -o /dev/null --max-time 1 "http://127.0.0.1:$ACTUAL_CDP_PORT/json/version" 2>/dev/null; then
			if [[ "$ACTUAL_CDP_PORT" != "$CDP_PORT" ]]; then
				echo "[launch.sh] note: Chromium bound CDP on $ACTUAL_CDP_PORT (requested $CDP_PORT was unavailable)" >&2
			fi
			CDP_PORT="$ACTUAL_CDP_PORT"
			READY=1
			echo "[launch.sh] CDP ready after ${i}s on port $CDP_PORT" >&2
			break
		fi
	fi
	sleep 1
...
if [[ "$READY" != "1" ]]; then
	echo "[launch.sh] timed out waiting for CDP (requested $CDP_PORT). Log tail:" >&2
	tail -n 80 "$LOG_FILE" >&2
	exit 1
fi
