...
function hasAuthenticodeSignature(filePath: string): Promise<boolean> {
	return new Promise((resolve, reject) => {
		...
		proc.on('error', err => {
			// signtool.exe ships with the Windows SDK and isn't on PATH in minimal
			// (e.g. unsigned public) build environments. Treat its absence as
			// "no signature to strip" so rcedit can still patch the PE.
...
function prepareCopilotRipgrepShimTask(platform: string, arch: string, destinationFolderName: string) {
	...
	return async () => {
		// Public fork release builds don't materialize the private Copilot agent
		// SDK layout (produced by build/agent-sdk/produce.ts in the internal
		// pipeline), so the shim has nothing to stitch. Allow opting out.
		if (process.env['VSCODE_SKIP_COPILOT_SDK_SHIM']) {
			console.log(`[prepareCopilotRipgrepShim] VSCODE_SKIP_COPILOT_SDK_SHIM set — skipping Copilot SDK ripgrep shim`);
			return;
		}
