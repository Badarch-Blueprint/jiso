...
suite('ClaudeAgent', () => {
	...
	test('native transport: models populate from supportedModels() with no proxy start and no CAPI models() call', async () => {
		...
		await tick();
		// `default` is dropped, Opus ranks first (with a `[1m]` name suffix derived
		// from its alias id), and the synthetic `opusplan` / `fableplan` are appended last.
		assert.deepStrictEqual({
			...
			models: [
				...
				{ id: 'fableplan', name: 'Fable Plan (Sonnet execution)' },
			],
			proxyStarts: 0,
	...
	test('native models are fetched once on load and pinned — a transport flip round-trip republishes without re-enumerating', async () => {
		const { agent, sdk, configService } = createTestContext(disposables, { rootConfig: { claudeUseCopilotProxy: false } });
		sdk.supportedModelsResult = [
		...
		await tick();
		// Flip to proxy (clears the list — no GitHub token) and back to native:
		// the pinned snapshot is republished, no second enumeration subprocess.
		...
		assert.deepStrictEqual({
			supportedModelsCalls: sdk.supportedModelsCallCount,
			ids: agent.models.get().map(m => m.id),
		}, {
			supportedModelsCalls: 1,
			ids: ['sonnet', 'opusplan', 'fableplan'],
		});
	});
