...
suite('ClaudeSdkPipeline', () => {
	...
	suite('send with preflight', () => {

		/** {@link ControllableWarmQuery} that also captures the prompt iterable handed to `query()`. */
		class IterableCapturingWarmQuery extends ControllableWarmQuery {
			iterable: AsyncIterable<SDKUserMessage> | undefined;
			override query(prompt: string | AsyncIterable<SDKUserMessage>): Query {
				this.iterable = prompt as AsyncIterable<SDKUserMessage>;
				return super.query(prompt);
			}
		}

		test('yields the preflight message to the SDK ahead of the prompt (auto-compact ordering)', async () => {
			const warm = new IterableCapturingWarmQuery();
			const { pipeline } = createPipeline(disposables, warm);

			pipeline.send(makePrompt('p1'), 'turn-A', makePrompt('compact')).catch(() => { /* unwound on teardown */ });
			await flushMicrotasks();

			// The SDK stub never drains the prompt iterable, so the test can pull
			// from it directly and observe the exact order the SDK would see.
			const it = warm.iterable![Symbol.asyncIterator]();
			const first = await it.next();
			const second = await it.next();
			assert.deepStrictEqual(
				[first.value?.uuid, second.value?.uuid],
				[makeUuid('compact'), makeUuid('p1')],
			);

			// Clean teardown: let the parked consumer loop unwind before dispose.
			warm.queries[0].end();
			await flushMicrotasks();
		});
	});
