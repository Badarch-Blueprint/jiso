...
import type { InitializeParams, InitializeResult, PingParams, ReconnectParams, ReconnectResult, SubscribeParams, SubscribeResult, ResourceReadParams, ResourceReadResult, ResourceWriteParams, ResourceWriteResult, ResourceListParams, ResourceListResult, ResourceCopyParams, ResourceCopyResult, ResourceDeleteParams, ResourceDeleteResult, ResourceMoveParams, ResourceMoveResult, ResourceResolveParams, ResourceResolveResult, ResourceMkdirParams, ResourceMkdirResult, ResourceRequestParams, ResourceRequestResult, UnsubscribeParams, DispatchActionParams, AuthenticateParams, AuthenticateResult, GetPlanUsageParams, IClaudePlanUsage, GetContextUsageParams, IClaudeContextUsage, GetSessionLogsParams, IClaudeSessionLogs, PingAgentParams } from './commands.js';
...
export interface CommandMap {
	...
	/** FORK: fetch the local Claude provider's subscription/plan usage (the `/usage` data). */
	'getPlanUsage': { params: GetPlanUsageParams; result: IClaudePlanUsage | undefined };
	/** FORK: fetch a session's live context-window occupancy (the `↓ N tokens` indicator). */
	'getContextUsage': { params: GetContextUsageParams; result: IClaudeContextUsage | undefined };
	/** FORK: fetch a session's harness-diagnostics log (context trimming / auto-compact / context usage). */
	'getSessionLogs': { params: GetSessionLogsParams; result: IClaudeSessionLogs | undefined };
	/** FORK: run one minimal throwaway turn to start the subscription's 5-hour usage window. */
	'pingAgent': { params: PingAgentParams; result: boolean };
}
