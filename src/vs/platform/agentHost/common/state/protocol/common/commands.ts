...
/**
 * FORK: params for the `getPlanUsage` command. Root-scoped, no extra fields —
 * the host resolves the local Claude provider's subscription/plan usage.
 */
export interface GetPlanUsageParams extends BaseParams {
	channel: 'ahp-root://';
}

/**
 * FORK: params for the `pingAgent` command. Root-scoped — runs one minimal
 * throwaway turn on the native Claude transport so the subscription's 5-hour
 * usage window starts ticking immediately.
 */
...
/**
 * FORK: params for the `getContextUsage` command. Session-scoped — `channel` is the session
 * resource URI whose live query is queried for its current context-window occupancy.
 */
export interface GetContextUsageParams extends BaseParams {
	channel: string;
}

/**
 * FORK: current context-window occupancy for a session — the data behind Claude Code's
 * `↓ N tokens` indicator (`Query.getContextUsage()`). `totalTokens` is the loaded context
 * (system prompt + tools + memory + skills + conversation), `maxTokens` the window size.
 */
/**
 * FORK: one row of the per-category context-window breakdown (system prompt, tools, MCP tools,
 * skills, messages, …) as reported by the SDK's `getContextUsage`. Lets the UI show where the
 * loaded context is actually spent rather than just a single total.
 */
export interface IClaudeContextUsageCategory {
	readonly name: string;
	readonly tokens: number;
	/** True for tool categories deferred behind tool search (loaded on demand, not in the prompt). */
	readonly isDeferred?: boolean;
}

export interface IClaudeContextUsage {
	readonly totalTokens: number;
	readonly maxTokens?: number;
	readonly percentage?: number;
	/** FORK: per-category breakdown of the loaded context. Omitted when the SDK doesn't report it. */
	readonly categories?: readonly IClaudeContextUsageCategory[];
}

/**
 * FORK: params for the `getSessionLogs` command. Session-scoped — `channel` is
 * the session resource URI whose harness-diagnostics log is fetched.
 */
export interface GetSessionLogsParams extends BaseParams {
	channel: string;
}

/**
 * FORK: recent harness-diagnostics log lines for a session — the visibility
 * surface for the fork's client-side context management (tool-output trimming,
 * read dedupe, auto-compact triggers, per-turn context-usage readings). Lines
 * are ISO-timestamped, oldest first, and capped by a per-session ring buffer.
 */
export interface IClaudeSessionLogs {
	readonly lines: readonly string[];
}

/** FORK: one model's session token/cost totals from the `/usage` view. */
export interface IClaudePlanModelUsage {
...
/** FORK: a named share of local usage (behavior key, skill or agent name → percent). */
export interface IClaudePlanShare {
	readonly name: string;
	readonly pct: number;
}

/** FORK: one model's consumption inside the locally-reconstructed 5-hour window. */
export interface IClaudeLocalWindowModelUsage {
	readonly model: string;
...
/**
 * FORK: the ACTIVE 5-hour usage window reconstructed from local `~/.claude`
 * transcripts (see `claudeLocalUsage.ts`). Fills the gap for plans whose
 * claude.ai endpoint returns `rate_limits: null` (e.g. team): Anthropic's
 * exact limit stays unknowable, but when the window started, when it resets,
 * and what was consumed inside it are all derivable locally. Same-machine
 * scope — a lower bound when other devices share the account.
 */
export interface IClaudeLocalWindowUsage {
	/** ISO 8601 — first request of the active window. */
	readonly windowStart: string;
	/** ISO 8601 — when the window resets (start + 5h). */
	readonly windowEnd: string;
	readonly requests: number;
	readonly models: readonly IClaudeLocalWindowModelUsage[];
}

/** FORK: a single rate-limit window from the Claude plan `/usage` view. */
export interface IClaudePlanUsageWindow {
	/** Percentage of the window consumed, 0-100. */
	readonly utilization: number;
	/** ISO 8601 timestamp when the window resets, if known. */
	readonly resetsAt?: string;
}

/**
 * FORK: structured Claude subscription/plan usage — the data behind the local
 * `claude` CLI `/usage` view. `available` is false for API-key / non-subscription
 * sessions, where the plan rate-limit windows do not apply.
 */
export interface IClaudePlanUsage {
	readonly available: boolean;
	readonly subscriptionType?: string;
	readonly sessionCostUsd?: number;
	/** FORK: session totals mirrored from the CLI `/usage` "Session" section. */
	readonly sessionApiDurationMs?: number;
	readonly sessionWallDurationMs?: number;
	readonly sessionLinesAdded?: number;
	readonly sessionLinesRemoved?: number;
	/** FORK: per-model token/cost breakdown for the session. */
	readonly modelUsage?: readonly IClaudePlanModelUsage[];
	readonly fiveHour?: IClaudePlanUsageWindow;
	readonly sevenDay?: IClaudePlanUsageWindow;
	readonly sevenDayOpus?: IClaudePlanUsageWindow;
	readonly sevenDaySonnet?: IClaudePlanUsageWindow;
	/**
	 * FORK: behavioral characteristics of the last 24h of local usage (what's
	 * contributing to limits: long context, cache misses, subagents, …).
	 * Categories overlap; percentages do not sum to 100.
	 */
	readonly dayBehaviors?: readonly IClaudePlanShare[];
	/** FORK: per-skill share of the last 24h of local usage. */
	readonly daySkills?: readonly IClaudePlanShare[];
	/** FORK: per-subagent share of the last 24h of local usage. */
	readonly dayAgents?: readonly IClaudePlanShare[];
	/**
	 * FORK: the active 5-hour window reconstructed from local transcripts —
	 * present regardless of whether the plan exposes `rate_limits`.
	 */
	readonly localWindow?: IClaudeLocalWindowUsage;
	/**
	 * Local-transcript activity counts (the same data the `/usage` dialog shows under
	 * "behaviors"). Present even when plan rate-limit windows are not (e.g. team plans,
	 * which return `rate_limits: null`). Approximate; this machine only.
	 */
	readonly dayRequests?: number;
	readonly daySessions?: number;
