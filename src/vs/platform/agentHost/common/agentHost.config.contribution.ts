...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		'chat.editor.defaultProvider': {
			type: 'string',
			enum: ['local', 'copilotEh', 'copilotAh', 'claudeCli', 'claude'],
			enumDescriptions: [
				...
				nls.localize('chat.editor.defaultProvider.claudeCli', "Use the legacy hand-rolled local Claude Code CLI (no interactive prompts)"),
				nls.localize('chat.editor.defaultProvider.claude', "Use the native local Claude agent (runs on your Claude login; supports permission prompts and questions)"),
			],
			...
			// FORK: default new chat sessions to the native local Claude agent (`claude`). It runs on
			// the user's local `claude` login (subscription) and supports interactive permission/question
			// prompts. No `experiment` override so this default is authoritative (must not re-point at Copilot).
			default: 'claude',
			tags: ['experimental'],
		},
