...
import type { CompletionsParams, CompletionsResult, CreateTerminalParams, IClaudeContextUsage, IClaudePlanUsage, ResolveSessionConfigResult, SessionConfigCompletionsResult } from './state/protocol/commands.js';
...
export function shouldSurfaceLocalAgentHostProvider(provider: AgentProvider, _configurationService: IConfigurationService, _isSessionsWindow: boolean): boolean {
	...
	// TEMP: unhidden to test the headless `claude` CLI transport in-IDE. Revert to
	// `return provider !== 'claude-cli';` when done.
	return true;
}
...
/**
 * FORK: protected resource for the native `claude` provider's own Anthropic credential
 * (the user's Claude subscription token from `claude setup-token`). Declared so the
 * `authenticate` RPC routes to {@link IAgent.authenticate}; it has no `authorization_servers`
 * so the workbench's automatic OAuth resolver finds no provider and skips it — the token is
 * supplied by the in-IDE sign-in flow and stored in SecretStorage, not via a GitHub session.
 */
export const ANTHROPIC_NATIVE_PROTECTED_RESOURCE: ProtectedResourceMetadata = {
	resource: 'anthropic-native-oauth',
	resource_name: 'Claude (Anthropic)',
	scopes_supported: [],
	required: false,
};
