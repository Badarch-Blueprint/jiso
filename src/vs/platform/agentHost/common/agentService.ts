...
import type { CompletionsParams, CompletionsResult, CreateTerminalParams, IClaudeContextUsage, IClaudePlanUsage, IClaudeSessionLogs, ResolveSessionConfigResult, SessionConfigCompletionsResult } from './state/protocol/commands.js';
 ...
 */
export function shouldSurfaceLocalAgentHostProvider(provider: AgentProvider, _configurationService: IConfigurationService, _isSessionsWindow: boolean): boolean {
	// FORK: `claude` (the native local Claude SDK provider) is the default agent and is always
	// surfaced — it runs on the user's local `claude` login, so it needs no opt-in setting. The
	// legacy hand-rolled `claude-cli` provider is superseded by it and hidden from the picker/list;
	// both read the same ~/.claude transcripts, so surfacing only `claude` avoids duplicate listings
	// AND keeps new sessions on the native provider (the only one with harness context management:
	// tool-output trimming, auto-compact, diagnostics). The temporary unhide for testing the
	// headless CLI transport let new sessions silently default to `claude-cli`, bypassing all of it.
	return provider !== 'claude-cli';
}

/**
 * FORK: both local Claude providers (`claude` — the native SDK transport — and
 * the legacy `claude-cli`) authenticate via the user's own `claude` login;
 * neither needs GitHub Copilot sign-in, and UI (chat gating, model picker)
 * must never offer a Copilot sign-in for their sessions.
 */
...
export interface IAgentCreateSessionConfig {
	...
	readonly workingDirectory?: URI;
	/**
	 * FORK: extra directories the session may access beyond `workingDirectory`.
	 * Used for multi-root workspaces so one chat session can read/edit every
	 * workspace folder (e.g. a backend + frontend repo). Projected onto the
	 * Claude SDK's `additionalDirectories`.
	 */
...
export interface IAgent {
	...
	/**
	 * FORK: structured subscription/plan usage (the `/usage` data: 5-hour and
	 * weekly rate-limit windows, per-model, session cost). Optional — only the
	 * local native Claude provider implements it; others omit it.
	 */
	getPlanUsage?(): Promise<IClaudePlanUsage | undefined>;

	/**
	 * FORK: current context-window occupancy for a live session — the data behind Claude Code's
	 * `↓ N tokens` indicator. Optional; only the native Claude provider implements it.
	 */
	getContextUsage?(session: URI): Promise<IClaudeContextUsage | undefined>;

	/**
	 * FORK: recent harness-diagnostics log lines for a session (context
	 * trimming, auto-compact, context usage). Optional; only the native
	 * Claude provider implements it.
	 */
	getSessionLogs?(session: URI): Promise<IClaudeSessionLogs | undefined>;

	/**
	 * FORK: run one minimal throwaway turn so the subscription's 5-hour usage
	 * window starts ticking now. Optional; only the native Claude provider
	 * implements it. Resolves `true` when the turn completed.
	 */
...
export interface IAgentService {
	...
	/**
	 * FORK: subscription/plan usage for the local native Claude provider (the
	 * `/usage` data: 5-hour & weekly rate-limit windows, per-model, session cost).
	 * Resolves `undefined` when no provider can report it.
	 */
	getPlanUsage(): Promise<IClaudePlanUsage | undefined>;

	/** FORK: live context-window occupancy for a session (the `↓ N tokens` indicator); `undefined` when unavailable. */
	getContextUsage(session: URI): Promise<IClaudeContextUsage | undefined>;
...
export interface IAgentConnection {
	 ...
	 */
	getCompletionTriggerCharacters(): Promise<readonly string[]>;
	/** FORK: subscription/plan usage (`/usage` data) for the local native Claude provider; `undefined` when unavailable. */
	getPlanUsage(): Promise<IClaudePlanUsage | undefined>;
	/** FORK: live context-window occupancy for a session (the `↓ N tokens` indicator); `undefined` when unavailable. */
	getContextUsage(session: URI): Promise<IClaudeContextUsage | undefined>;
	/** FORK: harness-diagnostics log for a session (context trimming / auto-compact / context usage); `undefined` when unavailable. */
