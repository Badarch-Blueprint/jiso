...
async function startAgentHost(): Promise<void> {
	...
	try {
		...
		// FORK: `@anthropic-ai/claude-agent-sdk` is a production dependency, so
		// packaged builds carry it in `node_modules` — accept that as a third way
		// for the SDK to be reachable (no `product.agentSdks.claude` needed).
		if (isAgentEnabled(process.env[AgentHostClaudeAgentEnabledEnvVar], true) && (!environmentService.isBuilt || agentSdkDownloader.isAvailable(ClaudeSdkPackage) || isClaudeSdkModuleBundled())) {
		...
		// FORK: register the Claude Code headless CLI provider. Drives the user's
		// local `claude` binary directly (their own login), independent of the
