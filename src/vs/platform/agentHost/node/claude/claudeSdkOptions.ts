...
import type { HookCallback, McpSdkServerConfigWithInstance, Options } from '@anthropic-ai/claude-agent-sdk';
...
export interface IBuildOptionsInput {
	...
	/**
	 * FORK: extra absolute directories the SDK may access beyond `workingDirectory`
	 * (multi-root workspaces). Projected onto `Options.additionalDirectories`.
	 * Omitted from the options when empty so the SDK keeps its default.
	 */
	...
	/**
	 * FORK: `PostToolUse` hook for client-side context management (see
	 * {@link createPostToolUseTrimHook}). Projected onto `Options.hooks` under
	 * the `PostToolUse` event. Omitted from the returned options when absent so
	 * the SDK keeps its default (no hooks). Pure harness behavior — no wire
	 * change — so it is transport-independent and always safe to attach.
	 */
...
export async function buildOptions(
	...
	const settingsEnv: Record<string, string> = {
		...
		// per-session bearer. Native mode omits both so the CLI runs its own
		// credential resolution exactly as it would in the user's terminal
		// (their `claude` login from Keychain/`~/.claude`, or any credential
		// they export in their shell — env forwarded by `buildSubprocessEnv`;
		// the IDE injects nothing and nothing sits on the wire).
	...
	return {
		...
		// FORK: only skip permission checks in explicit "Bypass Permissions" mode. Otherwise this
		// short-circuited `canUseTool`, so "Ask Before Edits" (and Plan/Auto/Edit-Automatically)
		// never prompted and every edit went through silently.
		...
		// FORK: `Task`/`Agent` spawn sub-agents (see `SUBAGENT_TOOL_NAMES`). Each sub-agent is a
		// separate context with its own cold prefix + turn loop, multiplying token cost; disallow
		// them so the IDE never fans out to sub-agents. `WebSearch` stays disallowed as before.
		disallowedTools: ['WebSearch', 'Task', 'Agent'],
		...
		...(input.postToolUseHook ? { hooks: { PostToolUse: [{ hooks: [input.postToolUseHook] }] } } : {}),
 ...
 *
 * - **Native, `false`:** inherit the real `process.env` so the subprocess sees
 *   the same environment as the user's terminal — `HOME` (for the CLI's own
 *   `claude` login credential in Keychain/`~/.claude`), `PATH`, and any
 *   credential the user exports themselves. The IDE adds no credential of its
 *   own. Without this spread, replace semantics hide the user's login and the
