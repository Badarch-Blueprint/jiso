 ...
 */
/**
 * FORK: removes context the workbench folds into the prompt at send time (and which `claude`
 * therefore stores in its transcript) so a RESTORED user message reads like what the user actually
 * typed. Live chat never shows this — it renders the clean request model; only the replayed
 * transcript carries the injected text. Applied here so every provider that replays Claude
 * transcripts (CLI and SDK) shares the cleanup. Pattern-based — extend as new injected blocks appear.
 */
function stripInjectedContext(text: string): string {
	let cleaned = text;
	// Injected `<system-reminder>…</system-reminder>` blocks (e.g. the file-reference reminder).
	cleaned = cleaned.replace(/\n*<system-reminder>[\s\S]*?<\/system-reminder>/g, '');
	// Trailing browser-sharing workspace context (browserTools.contribution.ts).
	for (const marker of ['No browser pages are currently shared with you.', 'The following browser pages are currently shared with you']) {
...
type ParsedSessionMessage =
	...
	| { readonly kind: 'assistant'; readonly uuid: string; readonly blocks: readonly AssistantBlock[]; readonly isInner: boolean; readonly usage: UsageInfo | undefined }
...
function parseAssistantMessage(msg: SessionMessage): ParsedSessionMessage | undefined {
	...
	return { kind: 'assistant', uuid: msg.uuid, blocks, isInner: msg.parent_tool_use_id !== null, usage: readAssistantUsage(msg.message) };
}

/**
 * Extracts per-turn token usage from a stored assistant envelope's `message.usage`
 * (`input_tokens` / `output_tokens` / `cache_read_input_tokens`) so restored turns carry
 * usage for cumulative token displays. Returns undefined when no usage is recorded.
 */
function readAssistantUsage(message: unknown): UsageInfo | undefined {
...
interface InProgressTurn {
	...
	/** Token usage from this turn's last assistant envelope (most recent wins). */
...
class ReplayBuilder {
	...
	private _consumeAssistant(msg: ParsedSessionMessage & { kind: 'assistant' }): void {
		...
		if (msg.usage) {
			// Last assistant envelope in the turn wins — reflects the turn's final token counts.
			this._active.usage = msg.usage;
		}
	...
	private _closeActive(): void {
		...
		const turn: Turn = {
			id: a.id,
			message: { text: stripInjectedContext(a.userText), origin: { kind: MessageKind.User } },
