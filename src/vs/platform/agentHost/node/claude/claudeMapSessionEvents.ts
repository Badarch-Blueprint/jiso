	...
	switch (message.type) {
		...
		default:
			...
			if (message.type === 'system') {
				// FORK: surface conversation compaction (from `/compact` or auto-compact) as a
				// visible transcript notice. The SDK delivers it as a `compact_boundary` system
				// message that would otherwise be dropped — leaving no sign the context was
				// summarized. This is the only user-visible signal that `/compact` did anything.
				if (message.subtype === 'compact_boundary') {
					return mapCompactBoundary(message, chat, turnId);
				}
...
/**
 * FORK: render an SDK {@link SDKCompactBoundaryMessage} as a Markdown notice in the
 * transcript so manual `/compact` (and auto-compaction) are visible, with the
 * trigger and before/after token counts when the SDK reports them.
 */
function mapCompactBoundary(
	...
	const fmt = (n: number | undefined) => typeof n === 'number' ? n.toLocaleString('en-US') : undefined;
	const pre = fmt(meta?.pre_tokens);
	const post = fmt(meta?.post_tokens);
	const detail = pre && post ? ` · ${pre} → ${post} tokens` : pre ? ` · was ${pre} tokens` : '';
	const content = `\n\n---\n\n**Context compacted** (${trigger})${detail} — earlier conversation summarized to free up context.\n\n---\n\n`;
	return [{
		kind: 'action',
		resource: chat,
		action: {
