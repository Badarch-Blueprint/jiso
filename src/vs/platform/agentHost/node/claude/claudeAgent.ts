...
import type { ModelInfo, Options, SDKControlGetUsageResponse, SDKSessionInfo, SDKUserMessage } from '@anthropic-ai/claude-agent-sdk';
import type { IClaudeContextUsage, IClaudePlanUsage, IClaudePlanUsageWindow, ResolveSessionConfigResult, SessionConfigCompletionsResult } from '../../common/state/protocol/commands.js';
...
export class ClaudeAgent extends Disposable implements IAgent {
	...
	/** FORK: the user's own Claude token (native transport), delivered by the in-IDE sign-in flow. */
	private _claudeOAuthToken: string | undefined;
	...
	getProtectedResources(): ProtectedResourceMetadata[] {
		...
		if (this._transportMode !== 'proxy') {
			// FORK: native mode authenticates with the user's own Claude token, delivered by the
			// in-IDE sign-in flow via the `authenticate` RPC (routed here because we declare it).
			return [ANTHROPIC_NATIVE_PROTECTED_RESOURCE, GITHUB_REPO_PROTECTED_RESOURCE];
		}
		return [
	...
	async authenticate(resource: string, token: string): Promise<boolean> {
		if (resource === ANTHROPIC_NATIVE_PROTECTED_RESOURCE.resource) {
			// FORK: the in-IDE "Sign in to Claude" flow delivers the user's own Claude token here.
			// The native SDK subprocess inherits `process.env` (see `buildSubprocessEnv`), so exposing
			// it as CLAUDE_CODE_OAUTH_TOKEN is how the SDK picks up the credential. Future sessions only.
			const changed = this._claudeOAuthToken !== token;
			this._claudeOAuthToken = token;
			if (token) {
				process.env.CLAUDE_CODE_OAUTH_TOKEN = token;
			} else {
				delete process.env.CLAUDE_CODE_OAUTH_TOKEN;
			}
			if (changed && this._transportMode !== 'proxy') {
				void this._refreshModels();
			}
			return true;
		}
	...
	private async _fetchNativeModels(): Promise<readonly IAgentModelInfo[]> {
		...
		try {
			...
			// FORK: KEEP the SDK's "Default (recommended)" / auto model (value `default`) and make it
			// the checked default (the picker treats `models[0]` as the default selection). This is the
			// CLI's Auto mode: it routes efficiently — the strong model for hard reasoning, cheaper
			// models for routine tool steps — instead of pinning pure Opus on every one of a task's
			// dozens of calls, so the same multi-step task costs far less than a concrete-Opus session.
			// (Previously this entry was filtered out, forcing users onto full-rate Opus for everything.)
			// Concrete models stay selectable, ordered Opus-first; within Opus the standard 200k variant
			// ranks ahead of the 1M `[1m]` one (a 1M window disables the SDK's auto-compaction — it only
			// fires near the limit, ~950k — and adds Anthropic's >200k long-context price premium).
			const isAuto = (m: ModelInfo) => m.value === 'default' || /recommended/i.test(m.displayName);
			const is1mVariant = (value: string) => /\[1m\]/i.test(value);
			const projected = models
				// The synthetic `opusplan` entry is prepended below; drop any SDK-listed duplicate.
				.filter(m => m.value !== 'opusplan')
				.sort((a, b) =>
					(Number(isAuto(b)) - Number(isAuto(a)))
					|| (Number(/opus/i.test(b.value)) - Number(/opus/i.test(a.value)))
					|| (Number(is1mVariant(a.value)) - Number(is1mVariant(b.value))))
				.map(m => fromSdkModelInfo(m, this.id));
			// FORK: prepend the CLI's `opusplan` tiered-routing mode as the checked default (models[0]).
