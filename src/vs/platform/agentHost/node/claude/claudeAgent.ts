...
import type { HookCallback, ModelInfo, Options, SDKControlGetUsageResponse, SDKSessionInfo, SDKUserMessage } from '@anthropic-ai/claude-agent-sdk';
import type { IClaudeContextUsage, IClaudePlanUsage, IClaudePlanUsageWindow, IClaudeSessionLogs, ResolveSessionConfigResult, SessionConfigCompletionsResult } from '../../common/state/protocol/commands.js';
...
import { FABLE_PLAN_MODEL_ID, tryParseClaudeModelId } from './claudeModelId.js';
import { resolvePromptToContentBlocks } from './claudePromptResolver.js';
import { computeLocalWindowUsage } from './claudeLocalUsage.js';
import { createPostToolUseTrimHook, IClaudeContextTrimConfig } from './claudeContextTrimmer.js';
...
export function fromSdkModelInfo(m: ModelInfo, provider: AgentProvider): IAgentModelInfo {
	...
	return {
		...
		id: m.value,
		name: m.displayName,
		// FORK: the SDK's native `ModelInfo` carries no vision flag, so this defaulted to
		// `false` and the chat rejected image attachments ("Opus does not support images").
		// Every current Claude model (Opus 4.x, Sonnet 4.x/5, Haiku 4.5) accepts image input,
		// so report vision support for the native lineup.
		supportsVision: true,
		// FORK: the SDK's native `ModelInfo` carries no token limits, so without this the chat
		// token gauge falls back to a 200k default — wrong for 1M-context models (it would read
		// e.g. "66%" when the context is really ~13% full). Infer the window from the id: the
		// `[1m]` suffix marks the 1M-context variant; everything else uses the standard 200k.
		maxContextWindow: /\[1m\]/i.test(m.value) ? 1_000_000 : 200_000,
		...(configSchema ? { configSchema } : {}),
	};
}

/** FORK: project one SDK rate-limit window into the workbench-facing shape. */
function toPlanUsageWindow(w: { utilization: number | null; resets_at: string | null } | null | undefined): IClaudePlanUsageWindow | undefined {
	if (!w || typeof w.utilization !== 'number') {
		return undefined;
	}
	return w.resets_at ? { utilization: w.utilization, resetsAt: w.resets_at } : { utilization: w.utilization };
}

/** FORK: project the experimental SDK `/usage` payload into {@link IClaudePlanUsage}. */
function projectPlanUsage(raw: SDKControlGetUsageResponse): IClaudePlanUsage {
	const rl = raw.rate_limits;
	const fiveHour = toPlanUsageWindow(rl?.five_hour);
	const sevenDay = toPlanUsageWindow(rl?.seven_day);
	const sevenDayOpus = toPlanUsageWindow(rl?.seven_day_opus);
	const sevenDaySonnet = toPlanUsageWindow(rl?.seven_day_sonnet);
	// `behaviors` carries local-transcript activity counts — the only "usage" numbers available for
	// plans that return no rate-limit windows (e.g. team plans, `rate_limits: null`).
	const day = raw.behaviors?.day;
	const week = raw.behaviors?.week;
	// FORK: session totals + per-model breakdown + day-window behavioral
	// insights, mirroring what the CLI `/usage` view renders.
	const modelUsage = raw.session?.model_usage
		? Object.entries(raw.session.model_usage).map(([model, u]) => ({
			model,
		...
		: undefined;
	const dayBehaviors = day?.behaviors?.map(b => ({ name: b.key, pct: b.pct }));
	const daySkills = day?.skills?.map(s => ({ name: s.name, pct: s.pct }));
	const dayAgents = day?.agents?.map(a => ({ name: a.name, pct: a.pct }));
	return {
		available: raw.rate_limits_available,
		...(raw.subscription_type ? { subscriptionType: raw.subscription_type } : {}),
		...(typeof raw.session?.total_cost_usd === 'number' ? { sessionCostUsd: raw.session.total_cost_usd } : {}),
		...(typeof raw.session?.total_api_duration_ms === 'number' ? { sessionApiDurationMs: raw.session.total_api_duration_ms } : {}),
		...(typeof raw.session?.total_duration_ms === 'number' ? { sessionWallDurationMs: raw.session.total_duration_ms } : {}),
		...(typeof raw.session?.total_lines_added === 'number' ? { sessionLinesAdded: raw.session.total_lines_added, sessionLinesRemoved: raw.session.total_lines_removed } : {}),
		...(modelUsage && modelUsage.length > 0 ? { modelUsage } : {}),
		...(fiveHour ? { fiveHour } : {}),
		...(sevenDay ? { sevenDay } : {}),
		...(sevenDayOpus ? { sevenDayOpus } : {}),
		...(sevenDaySonnet ? { sevenDaySonnet } : {}),
		...(dayBehaviors && dayBehaviors.length > 0 ? { dayBehaviors } : {}),
		...(daySkills && daySkills.length > 0 ? { daySkills } : {}),
		...(dayAgents && dayAgents.length > 0 ? { dayAgents } : {}),
		...(typeof day?.request_count === 'number' ? { dayRequests: day.request_count, daySessions: day.session_count } : {}),
		...(typeof week?.request_count === 'number' ? { weekRequests: week.request_count, weekSessions: week.session_count } : {}),
	};
}
...
export class ClaudeAgent extends Disposable implements IAgent {
	...
	/**
	 * FORK: the native model list, fetched ONCE on initial load and pinned for
	 * the host's lifetime (see {@link _refreshModels}). Sessions and later
	 * events never change the list — they republish this snapshot.
	 */
	private _pinnedNativeModels: readonly IAgentModelInfo[] | undefined;
	private _proxyHandle: IClaudeProxyHandle | undefined;
	/**
	 * FORK: client-side context management — a shared `PostToolUse` hook that
	 * caps oversized tool outputs (and optionally dedups repeated reads)
	 * before they enter a session's transcript. One instance serves every
	 * session: its internal dedupe state is keyed per session + subagent, and
	 * its config getter reads the live root config on each tool call, so
	 * setting changes apply immediately without a restart. Pure harness
	 * behavior — nothing on the wire changes.
	 */
	private readonly _postToolUseTrimHook: HookCallback;
	...
	constructor(
		...
		this._postToolUseTrimHook = createPostToolUseTrimHook(
			() => this._resolveContextTrimConfig(),
			(message, sessionId) => {
				this._logService.info(message);
				// FORK: also record into the session's diagnostics log so the
				// chat's "Session Logs" view shows what the trimmer did.
				this._findAnySession(sessionId)?.appendDiagnostic(message);
			},
		);
	...
	private _resolveTransportMode(): 'proxy' | 'native' {
		// FORK: default to native when the `claudeUseCopilotProxy` root value is unset, so Claude
		// runs on the user's local `claude` login (subscription) rather than the Copilot proxy.
	...
	getProtectedResources(): ProtectedResourceMetadata[] {
		...
		if (this._transportMode !== 'proxy') {
			// FORK: native mode carries no Anthropic credential of its own — the spawned
			// `claude` subprocess inherits the user's environment and authenticates with the
			// user's own Claude Code login (`/login` in a terminal), exactly like the CLI.
	...
	/** FORK: client-side trim tuning from the root config (see `claudeContextTrimmer.ts`). */
	private _resolveContextTrimConfig(): IClaudeContextTrimConfig {
		const enabled = this._configurationService.getRootValue(agentHostCustomizationConfigSchema, AgentHostConfigKey.ClaudeLocalContextTrim) ?? true;
		const maxChars = this._configurationService.getRootValue(agentHostCustomizationConfigSchema, AgentHostConfigKey.ClaudeLocalContextTrimMaxChars) ?? 20000;
		const dedupeReads = this._configurationService.getRootValue(agentHostCustomizationConfigSchema, AgentHostConfigKey.ClaudeLocalContextDedupeReads) ?? false;
		return { enabled, maxChars, dedupeReads };
	}

	/**
	 * FORK: resolve the transport for a session startup. Native sessions always
	 * connect to Anthropic directly; nothing sits on the wire.
	 */
	private async _resolveTransport(): Promise<ClaudeTransport> {
	...
	private async _refreshModels(): Promise<void> {
		...
		// FORK: native models are fetched once (the initial-load fetch kicked off in the
		// constructor) and then pinned — later triggers (transport flip round-trips, auth
		// events) republish the pinned snapshot instead of re-enumerating, so nothing that
		// happens after startup can change the model list. A failed/empty initial fetch
		// does not pin, so the next trigger retries.
		if (!proxyAtStart && this._pinnedNativeModels) {
			this._models.set(this._pinnedNativeModels, undefined);
			return;
		}
		try {
			...
			// Pin only a real enumeration: the synthetic `opusplan` / `fableplan` are
			// always appended, so require at least one SDK-fetched model before
			// freezing the list.
			if (!proxyAtStart && filtered.some(m => m.id !== 'opusplan' && m.id !== FABLE_PLAN_MODEL_ID)) {
				this._pinnedNativeModels = filtered;
			}
	...
	private async _fetchNativeModels(): Promise<readonly IAgentModelInfo[]> {
		...
		try {
			...
			// FORK: drop the SDK's synthetic "Default (recommended)" auto entry (value `default`).
			// The picker treats `models[0]` as the checked default, and the ranking below decides
			// that deliberately — concrete models only, no CLI auto-routing pseudo-model.
			const is1mVariant = (value: string) => /\[1m\]/i.test(value);
			// FORK: stable picker order (and `models[0]` = checked default): Opus first, then
			// Fable, Sonnet, Haiku; a 1M-window variant ranks after its standard sibling (a 1M
			// window disables the SDK's auto-compaction — it only fires near the limit — and adds
			// Anthropic's >200k long-context price premium). Matches both dated ids
			// (`claude-opus-4-8…`) and the aliases the SDK also returns (`opus[1m]`, `sonnet`).
			// Only the sort rank and a `[1m]` name suffix are derived — never the `id`
			// (`m.value`), so an unmatched SDK id can never break selection; it keeps its SDK
			// name and falls to the bottom ("Other Models").
			const rank = (id: string): number => {
				const late = is1mVariant(id) ? 1 : 0;
				if (/opus/i.test(id)) { return 0 + late; }
				if (/fable/i.test(id)) { return 10 + late; }
				if (/sonnet/i.test(id)) { return 20 + late; }
				if (/haiku/i.test(id)) { return 30 + late; }
				return 100;
			};
			const projected = models
				// The synthetic `opusplan` / `fableplan` entries are appended below; drop any SDK-listed duplicate.
				.filter(m => m.value !== 'default' && m.value !== 'opusplan' && m.value !== FABLE_PLAN_MODEL_ID)
				.map(m => {
					const info = fromSdkModelInfo(m, this.id);
					// Disambiguate 1M variants whose SDK display name doesn't say so
					// (e.g. `opus[1m]` is displayed as just "Opus").
					const name = is1mVariant(info.id) && !/1m/i.test(info.name) ? `${info.name} [1m]` : info.name;
					return { info: { ...info, name }, rank: rank(info.id) };
				})
				.sort((a, b) => a.rank - b.rank)
				.map(x => x.info);
			// FORK: keep the CLI's `opusplan` tiered-routing mode available (appended at the end below).
			// It uses Opus for high-level planning and the much cheaper Sonnet for the verbose, repetitive
			// execution steps of an agentic loop — so a long multi-step task (e.g. a 38-tool-call debug run)
			// costs a fraction of pure Opus with no meaningful quality loss on execution. Verified: `--model
			// opusplan` is accepted by the SDK and routes tool-step generation to claude-sonnet-5. The `id`
			// passes through `toSdkModelId` unchanged (no claude-* pattern match). Auto and concrete Opus/
			// Sonnet stay selectable below it for users who want pure-Opus quality on every step.
			const opusPlan: IAgentModelInfo = {
				provider: this.id,
				id: 'opusplan',
				name: 'Opus Plan (Sonnet execution)',
				supportsVision: true,
				maxContextWindow: 200_000,
			};
			// FORK: `fableplan` — same tiered idea one tier up: Fable for plan-mode
			// reasoning, Sonnet for execution steps. Unlike `opusplan` this is NOT a
			// CLI alias (2.1.199's accepted model settings end at `opusplan`), so the
			// id must never reach the SDK: ClaudeAgentSession resolves it to `fable` /
			// `sonnet` per the live permission mode (see resolveTieredModelId). The
			// raw id persists only in session metadata / the picker selection.
			const fablePlan: IAgentModelInfo = {
				provider: this.id,
				id: FABLE_PLAN_MODEL_ID,
				name: 'Fable Plan (Sonnet execution)',
				supportsVision: true,
				maxContextWindow: 200_000,
			};
			// FORK: curated four lead (Opus [200k] is models[0] = checked default); Opus Plan,
			// Fable Plan and Auto stay available but no longer pre-empt the user's picked default.
			return [...projected, opusPlan, fablePlan];
		} finally {
	...
	/**
	 * FORK: fetch the user's Claude subscription/plan usage (the `/usage` data:
	 * 5-hour & weekly rate-limit windows, per-model, session cost). Native only —
	 * plan limits don't exist for the Copilot-proxy transport. Opens an ephemeral
	 * query (same pattern as {@link _fetchNativeModels}; reads `~/.claude`, runs no
	 * turn) and calls the experimental SDK usage control request, guarded so an
	 * SDK shape change degrades to `undefined` rather than throwing.
	 */
	async getPlanUsage(): Promise<IClaudePlanUsage | undefined> {
		if (this._isProxyEnabled()) {
			return undefined;
		}
		const neverYieldingPrompt: AsyncIterable<SDKUserMessage> = {
			[Symbol.asyncIterator]: () => ({ next: () => new Promise<IteratorResult<SDKUserMessage>>(() => { /* never resolves */ }) }),
		...
		const query = await this._sdkService.query({ prompt: neverYieldingPrompt, options });
		try {
			// FORK: run the local 5-hour-window reconstruction alongside the SDK
			// usage request — it fills the windows gap for plans whose endpoint
			// returns `rate_limits: null` (e.g. team). Local file reads only;
			// failure degrades to omitting the section.
			const [raw, localWindow] = await Promise.all([
				query.usage_EXPERIMENTAL_MAY_CHANGE_DO_NOT_RELY_ON_THIS_API_YET(),
				computeLocalWindowUsage(),
			]);
			const projected = projectPlanUsage(raw);
			return localWindow ? { ...projected, localWindow } : projected;
		} catch (err) {
			this._logService.warn('[Claude] getPlanUsage failed', err);
			return undefined;
		} finally {
			query.close();
			options.abortController?.abort();
	...
	/**
	 * FORK: current context-window usage for a live session — Claude Code's `↓ N tokens` number.
	 * Unlike {@link getPlanUsage} (ephemeral query), this asks the session's LIVE query so the count
	 * includes the conversation. Returns undefined when the session isn't materialized or the
	 * control request fails.
	 */
	async getContextUsage(session: URI): Promise<IClaudeContextUsage | undefined> {
		const liveSession = this._findAnySession(AgentSession.id(session));
		if (!liveSession) {
			return undefined;
		}
		try {
			const raw = await liveSession.getContextUsage();
			if (!raw) {
				return undefined;
			}
			return {
				...
				// FORK: pass through the SDK's per-category breakdown (system prompt, tools, MCP, skills,
				// messages…) so the context-usage panel can show where the loaded context is spent.
				...(raw.categories && raw.categories.length > 0
					? { categories: raw.categories.map(c => ({ name: c.name, tokens: c.tokens, ...(c.isDeferred ? { isDeferred: true } : {}) })) }
					: {}),
			};
		} catch (err) {
			this._logService.warn('[Claude] getContextUsage failed', err);
			return undefined;
		}
	}

	/**
	 * FORK: minimal billable ping — runs a single throwaway "hi" turn on the
	 * cheapest model via an ephemeral query (no session, no transcript in the
	 * IDE) so the subscription's 5-hour usage window starts ticking NOW rather
	 * than on the user's first real prompt. Native only — plan windows don't
	 * exist on the Copilot-proxy transport. Resolves `true` when the turn
	...
	async pingAgent(): Promise<boolean> {
		...
		const options = { ...buildModelEnumerationOptions(), model: 'haiku', maxTurns: 1 };
		const query = await this._sdkService.query({ prompt: 'hi', options });
		try {
			for await (const message of query) {
				if (message.type === 'result') {
					this._logService.info(`[Claude] pingAgent: result ${message.subtype} — 5-hour session window ${message.subtype === 'success' ? 'started' : 'NOT confirmed'}`);
					return message.subtype === 'success';
				}
			}
			return false;
		} catch (err) {
			this._logService.warn('[Claude] pingAgent failed', err);
			return false;
		} finally {
			query.close();
			options.abortController?.abort();
	...
	/**
	 * FORK: recent harness-diagnostics log lines for a live session — what the
	 * fork's client-side context management actually did (tool-output trims,
	 * read dedupes, auto-compact triggers, per-turn context-usage readings).
	 * Returns undefined when the session isn't materialized in this host.
	 */
	async getSessionLogs(session: URI): Promise<IClaudeSessionLogs | undefined> {
		const liveSession = this._findAnySession(AgentSession.id(session));
		if (!liveSession) {
			return undefined;
		}
		return { lines: [...liveSession.diagnosticsLog] };
	}
	...
	private async _materializeProvisional(sessionId: string): Promise<ClaudeAgentSession> {
		...
		try {
			await session.materialize({ transport, canUseTool, isResume: false, serverToolHost: this._serverToolHost, postToolUseHook: this._postToolUseTrimHook });
		} catch (err) {
			this._sessions.deleteAndDispose(sessionId);
			throw err;
		}
	...
	private async _resumeSession(sessionId: string, sessionUri: URI): Promise<ClaudeAgentSession> {
		...
		const session = ClaudeAgentSession.createProvisional(
			...
			sessionUri,
			workingDirectory,
			// FORK: additional dirs aren't persisted in the SDK transcript, so a
			// session resumed from disk after a restart starts with primary-folder
			// access only until the next fresh create. (multi-root note)
			[],
			project,
			overlay.model,
			overlay.agent,
		...
		try {
			await session.materialize({ transport, canUseTool, isResume: true, serverToolHost: this._serverToolHost, postToolUseHook: this._postToolUseTrimHook });
		} catch (err) {
			this._sessions.deleteAndDispose(sessionId);
			throw err;
		}
	...
	resolveSessionConfig(_params: IAgentResolveSessionConfigParams): Promise<ResolveSessionConfigResult> {
		...
		const sessionSchema = createSchema({
			[ClaudeSessionConfigKey.PermissionMode]: schemaProperty<ClaudePermissionMode>({
				...
				// FORK: 'bypassPermissions' intentionally omitted from the picker (too
				// dangerous to surface). It stays in the ClaudePermissionMode union and
				// is still honored if a session already carries it.
	...
	async sendMessage(sessionUri: URI, _chat: URI, prompt: string, attachments?: readonly MessageAttachment[], turnId?: string): Promise<void> {
		...
		return this._sessionSequencer.queue(sessionId, async () => {
			...
			// FORK: never stack the auto-compact preflight ahead of an explicit
			// slash command (e.g. the user's own `/compact` or `/clear`) — the
			// command should run against the context the user is looking at.
			await session.send(sdkPrompt, effectiveTurnId, { allowAutoCompact: !prompt.trimStart().startsWith('/') });
		});
	}
