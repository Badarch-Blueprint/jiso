...
/**
 * FORK: IDE-synthetic tiered-routing pseudo-model — Fable for plan-mode
 * reasoning, Sonnet for execution steps. The CLI natively ships `opusplan`
 * ("Opus in plan mode, else Sonnet") but has no Fable analog (CLI 2.1.199's
 * accepted model settings end at `opusplan`), so the IDE defines its own.
 * The id must NEVER reach the SDK: every path that crosses into the pipeline
 * resolves it via {@link resolveTieredModelId} first. Only session metadata
 * and the model picker keep the raw `fableplan` id, so the user's selection
 * round-trips across resumes.
 */
export const FABLE_PLAN_MODEL_ID = 'fableplan';

/**
 * Resolve a tiered pseudo-model id to the concrete SDK model alias for the
 * given permission mode: `fableplan` becomes `fable` while in plan mode and
 * `sonnet` otherwise. All other ids pass through unchanged. The returned
 * aliases are SDK-native (the CLI resolves them to the current model
 * generation), so callers still route the result through
 * {@link toSdkModelId} like any other id.
 */
export function resolveTieredModelId(modelId: string, permissionMode: string | undefined): string;
export function resolveTieredModelId(modelId: string | undefined, permissionMode: string | undefined): string | undefined;
export function resolveTieredModelId(modelId: string | undefined, permissionMode: string | undefined): string | undefined {
	if (modelId === FABLE_PLAN_MODEL_ID) {
		return permissionMode === 'plan' ? 'fable' : 'sonnet';
	}
	return modelId;
}
