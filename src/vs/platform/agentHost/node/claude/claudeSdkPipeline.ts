...
import type { AgentInfo, McpServerStatus, PermissionMode, Query, SDKControlGetContextUsageResponse, SDKUserMessage, SlashCommand, WarmQuery } from '@anthropic-ai/claude-agent-sdk';
...
export class ClaudeSdkPipeline extends Disposable {
	...
	/**
	 * FORK: current context-window usage for this session's live query — the data behind Claude
	 * Code's `↓ N tokens` indicator. Retries briefly: right after a turn the control transport can
	 * momentarily report "not ready for writing" before the query parks between turns.
	 * `retryWhileSettling: false` makes exactly one attempt with no timers — for fire-and-forget
	 * callers (post-turn instrumentation) that must never leave a retry timer pending after
	 * session teardown.
	 */
	async getContextUsage(retryWhileSettling: boolean = true): Promise<SDKControlGetContextUsageResponse> {
		const query = await this._ensureQueryBound();
		let lastError: unknown;
		const attempts = retryWhileSettling ? 4 : 1;
		for (let attempt = 0; attempt < attempts; attempt++) {
			try {
				return await query.getContextUsage();
			} catch (err) {
				lastError = err;
				// Abort-aware retry delay: cancel the timer the moment the
				// pipeline aborts (dispose/teardown) so a pending retry never
				// outlives the pipeline. No delay after the final attempt.
				if (attempt === attempts - 1 || this._abortController.signal.aborted) {
					break;
				}
				const delay = timeout(500);
				const onAbort = () => delay.cancel();
				this._abortController.signal.addEventListener('abort', onAbort, { once: true });
				try {
					await delay;
				} catch {
					break; // cancelled by abort
				} finally {
	 ...
	 *
	 * FORK: `preflight` is an optional maintenance message (auto-compact's
	 * `/compact`) enqueued atomically AHEAD of the prompt under the SAME
	 * turnId, so its transcript output (e.g. the compact-boundary notice)
	 * renders inside the prompt's active protocol turn. Its intermediate
	 * `result` does not fire `ChatTurnComplete` (M10 batching: the queue
	 * still holds the prompt entry) and its deferred is observed by nobody
	 * — same contract as steering entries.
	 */
	async send(prompt: SDKUserMessage, turnId: string, preflight?: SDKUserMessage): Promise<void> {
		if (this._needsRebind) {
			await this._rebindQuery('recover');
		}
		...
		if (preflight) {
			const preflightEntry: IPendingSdkMessage = {
				sdkMessage: preflight,
				sdkUuid: typeof preflight.uuid === 'string' ? preflight.uuid : `${turnId}-preflight`,
				turnId,
				deferred: new DeferredPromise<void>(),
			};
			// Nobody awaits the preflight's deferred (the caller's promise is
			// the prompt entry's); swallow the `failAll` rejection on
			// abort/crash so it never surfaces as an unhandled rejection.
			this._queue.push(preflightEntry).catch(() => { /* expected on abort/crash */ });
		}
