...
export interface IAgentConfigurationService {
	...
	/**
	 * FORK: extra directories the session may access beyond its working
	 * directory (multi-root workspaces). Like {@link getEffectiveWorkingDirectory},
	 * falls back to the parent (subagent) session. Returns `[]` when none are set.
	 */
	getEffectiveAdditionalDirectories(session: ProtocolURI): string[];
...
export class AgentConfigurationService extends Disposable implements IAgentConfigurationService {
	...
	getEffectiveAdditionalDirectories(session: ProtocolURI): string[] {
		const own = this._stateManager.getSessionState(session)?.additionalDirectories;
		if (own !== undefined) {
			return own;
		}
		const parentInfo = parseSubagentSessionUri(session);
		if (parentInfo) {
			return this._stateManager.getSessionState(parentInfo.parentSession.toString())?.additionalDirectories ?? [];
		}
		return [];
	}
