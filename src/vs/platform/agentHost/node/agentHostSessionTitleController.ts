...
export interface IAgentHostSessionTitleControllerOptions {
	...
	/**
	 * Optional override for the small-model completion used to generate titles.
	 * When provided it is used instead of the GitHub Copilot utility model, so
	 * titles can be produced by the local Claude CLI with no Copilot dependency.
	 */
	readonly runUtilityPrompt?: (messages: ICopilotUtilityChatMessage[], token: CancellationToken) => Promise<string | undefined>;
}
...
export class AgentHostSessionTitleController extends Disposable {
	...
	/**
	 * FORK: master switch for LLM-based title generation. Disabled for now — each generation spawned
	 * a `claude -p` (or a Copilot utility completion) that carried the entire tool / MCP / skill
	 * prefix, costing ~20k billed per title on top of every turn. With this off the session title
	 * stays as the first user message (applied as the fallback by {@link seedTitleFromFirstMessage}).
	 * Set to `true` to re-enable; the invocation was also minimized (see `supportPrompt.ts`) so
	 * a re-enabled title costs ~200 tokens instead of ~20k — and it now runs on the configured
	 * support backend (`chatSupportBackend`), so pointing that at `agy` / `cursor-agent` keeps
	 * re-enabled title generation off the main Claude usage bucket entirely.
	 */
	private readonly _titleGenerationEnabled: boolean = false;
	...
	private async _generateTitleFromPrompt(promptContent: string, isConversation: boolean, token: CancellationToken): Promise<string | undefined> {
		// FORK: LLM title generation is disabled (see _titleGenerationEnabled). Returning undefined
		// here makes _generateTitle keep the seeded first-message fallback instead of refining it.
		if (!this._titleGenerationEnabled || token.isCancellationRequested) {
			return undefined;
		}

		const messages = this._buildTitlePrompt(promptContent, isConversation);

		// Prefer the injected generator (local Claude CLI) when available, so title
		// generation has no GitHub Copilot dependency.
		if (this._options.runUtilityPrompt) {
			const rawTitle = await this._options.runUtilityPrompt(messages, token);
			return rawTitle ? this._cleanTitle(rawTitle) : undefined;
		}
	...
	private _cleanTitle(rawTitle: string): string | undefined {
		...
		// Capitalize the first letter so titles read as proper sentence case
		// (the model often returns an all-lowercase leading word like "add ...").
