...
import type { ICopilotApiService, ICopilotUtilityChatMessage } from './shared/copilotApiService.js';
...
export interface IAgentSideEffectsOptions {
	...
	/** Optional small-model completion used for title generation instead of Copilot (e.g. local Claude CLI). */
...
export class AgentSideEffects extends Disposable {
	...
	constructor(
		...
		this._titleController = this._register(instantiationService.createInstance(AgentHostSessionTitleController, this._stateManager, {
			...
			runUtilityPrompt: this._options.runUtilityPrompt,
		}));
