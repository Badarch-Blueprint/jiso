...
export class SessionPermissionManager extends Disposable {
	...
	async getAutoApproval(e: IToolApprovalEvent, sessionKey: ProtocolURI): Promise<ToolCallConfirmationReason | undefined> {
		...
		// FORK: in a multi-root workspace one session may span several folders; treat
		// the working dir + every additional directory as the in-scope set for the
		// read/write path checks below, so edits in the second repo aren't forced to
		// prompt when they would auto-approve in the primary one.
		const scopeDirs: (string | undefined)[] = [workDir, ...this._configService.getEffectiveAdditionalDirectories(sessionKey)];
		const claudeMode = narrowClaudePermissionMode(this._configService.getSessionConfigValues(sessionKey)?.[ClaudeSessionConfigKey.PermissionMode]);
		...
		// 2b. FORK: "Auto Mode" (Claude permissionMode 'auto') means run tool
		// operations automatically. Since we only set `allowDangerouslySkipPermissions`
		// for 'bypassPermissions', the SDK still routes every tool through
		// `canUseTool` in auto mode — so without this the host would fall through to
		// the per-path/-command rules below and stall shell commands that don't match
		// an allow-rule. Auto-approve here to restore "let it run" behavior.
		if (claudeMode === 'auto') {
			return ToolCallConfirmationReason.Setting;
		}
		...
		if (e.permissionKind === 'read' && e.permissionPath) {
			if (scopeDirs.some(d => this._isPathInWorkingDirectory(e.permissionPath!, d))) {
		...
		if (e.permissionKind === 'write' && e.permissionPath) {
			// FORK: when the Claude session runs in a mode that means "ask before
			// edits" ("Ask Before Edits" → 'default', "Plan Mode" → 'plan'), the SDK
			// surfaces the write to `canUseTool` precisely so the user can confirm;
			// the host's path-based auto-approval must not override that and silently
			// apply the edit. ('auto' is already auto-approved above; 'acceptEdits'
			// keeps the path-based auto-approval; non-Claude sessions have an
			// `undefined` mode and so are unaffected.)
			if (claudeMode === 'default' || claudeMode === 'plan') {
				return undefined;
			}
			if (await this._isEditAutoApproved(e.permissionPath, scopeDirs)) {
		...
		if (e.permissionKind === 'shell' && e.toolInput) {
			...
			const result = this._commandAutoApprover.shouldAutoApprove(e.toolInput, {
				...
				isWriteDestApproved: (dest) => this._isShellWriteDestApproved(dest, workDir, scopeDirs),
			});
			if (result === 'approved') {
	 ...
	 */
	private _isShellWriteDestApproved(dest: string, workDir: string | undefined, scopeDirs: (string | undefined)[]): boolean {
		// Resolve relative redirect targets against the shell's cwd (the primary
		// working dir), then check the result against the full multi-root scope.
		const resolved = this._resolveShellRedirectPath(dest, workDir);
		if (!resolved) {
			return false;
		}
		return this._checkWritePath(resolved, scopeDirs);
	}
	 ...
	 */
	private async _isEditAutoApproved(filePath: string, scopeDirs: (string | undefined)[]): Promise<boolean> {
		const pathsToCheck = await this._resolveWritePaths(filePath);
		return pathsToCheck !== undefined && pathsToCheck.every(p => this._checkWritePath(p, scopeDirs));
	}
	 ...
	 * FORK: `scopeDirs` is the working dir plus any additional workspace folders;
	 * the path must live inside one of them. Platform-restriction is evaluated
	 * against the specific scope dir that contains the path.
	 */
	private _checkWritePath(filePath: string, scopeDirs: (string | undefined)[]): boolean {
		try {
			assertPathIsSafe(filePath);
		} catch {
			return false;
		}
		const containingDir = scopeDirs.find(d => this._isPathInWorkingDirectory(filePath, d));
		if (containingDir === undefined) {
			return false;
		}
		if (this._isPlatformRestrictedPath(filePath, containingDir)) {
			return false;
		}
