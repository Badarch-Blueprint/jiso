...
import type { CompletionsParams, CompletionsResult, CreateTerminalParams, IClaudeContextUsage, IClaudePlanUsage, IClaudeSessionLogs, ResolveSessionConfigResult, SessionConfigCompletionsResult } from '../common/state/protocol/commands.js';
...
import { runSupportPrompt, SupportBackend } from './claude/supportPrompt.js';
import { agentHostCustomizationConfigSchema, AgentHostConfigKey } from '../common/agentHostCustomizationConfig.js';
...
export class AgentService extends Disposable implements IAgentService {
	...
	constructor(
		...
		this._sideEffects = this._register(instantiationService.createInstance(AgentSideEffects, this._stateManager, {
			...
			// Generate session titles and other light "support" completions on the
			// configured backend. Routing them to an idle secondary CLI (agy /
			// cursor-agent) keeps them off the main chat's Claude usage bucket;
			// `claude` (the default) keeps them on the local headless CLI.
			runUtilityPrompt: (messages, token) => {
				const backend = (configurationService.getRootValue(agentHostCustomizationConfigSchema, AgentHostConfigKey.ChatSupportBackend) as SupportBackend | undefined) ?? 'claude';
				const model = configurationService.getRootValue(agentHostCustomizationConfigSchema, AgentHostConfigKey.ChatSupportModel) || undefined;
				return runSupportPrompt(backend, model, messages, token, this._logService);
			},
	...
	// FORK: route to the first provider that can report subscription/plan usage
	// (only the native local Claude provider implements `getPlanUsage`).
	async getPlanUsage(): Promise<IClaudePlanUsage | undefined> {
		for (const provider of this._providers.values()) {
			if (provider.getPlanUsage) {
				const usage = await provider.getPlanUsage();
				if (usage) {
					return usage;
				}
			}
	...
	// FORK: session-scoped. Iterate providers rather than `_findProviderForSession` — the browser
	// passes the session-type-scheme URI (`agent-host-claude:/<id>`) which doesn't map by provider
	// name; the owning provider resolves the session by its scheme-agnostic raw id (`AgentSession.id`).
	async getContextUsage(session: URI): Promise<IClaudeContextUsage | undefined> {
		for (const provider of this._providers.values()) {
			if (provider.getContextUsage) {
				const usage = await provider.getContextUsage(session);
				if (usage) {
					return usage;
				}
			}
	...
	// FORK: session-scoped harness-diagnostics log; same provider-iteration rationale as `getContextUsage`.
	async getSessionLogs(session: URI): Promise<IClaudeSessionLogs | undefined> {
		for (const provider of this._providers.values()) {
			if (provider.getSessionLogs) {
				const logs = await provider.getSessionLogs(session);
				if (logs) {
					return logs;
				}
			}
	...
	private _buildInitialSummary(provider: IAgent, session: URI, config: IAgentCreateSessionConfig | undefined, created: { project?: { uri: URI; displayName: string }; workingDirectory?: URI }, title: string): SessionSummary {
		...
		return {
			...
			...(config?.additionalDirectories && config.additionalDirectories.length > 0
				? { additionalDirectories: config.additionalDirectories.map(d => d.toString()) }
				: {}),
		};
	}
