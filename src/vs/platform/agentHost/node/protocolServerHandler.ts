...
export class ProtocolServerHandler extends Disposable {
	...
	private readonly _requestHandlers: RequestHandlerMap = {
		...
		createSession: async (_client, params) => {
			...
			try {
				createdSession = await this._agentService.createSession({
					...
					additionalDirectories: params.additionalDirectories?.map(d => URI.parse(d)),
		...
		getPlanUsage: async (_client, _params) => {
			return this._agentService.getPlanUsage();
		},
		getContextUsage: async (_client, params) => {
			return this._agentService.getContextUsage(URI.parse(params.channel));
		},
