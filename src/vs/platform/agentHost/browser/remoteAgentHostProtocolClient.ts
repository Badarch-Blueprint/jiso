...
import { ContentEncoding, ResourceRequestParams, type CompletionsParams, type CompletionsResult, type CreateTerminalParams, type IClaudeContextUsage, type IClaudePlanUsage, type IClaudeSessionLogs, type ResolveSessionConfigResult, type SessionConfigCompletionsResult } from '../common/state/protocol/commands.js';
...
export class RemoteAgentHostProtocolClient extends Disposable implements IAgentConnection {
	...
	createSession(config?: IAgentCreateSessionConfig): Promise<URI> {
		...
		const promise = this._sendRequest('createSession', {
			...
			additionalDirectories: config?.additionalDirectories?.map(d => fromAgentHostUri(d).toString()),
	...
	async getPlanUsage(): Promise<IClaudePlanUsage | undefined> {
		return this._sendRequest('getPlanUsage', { channel: ROOT_STATE_URI });
	}

	async getContextUsage(session: URI): Promise<IClaudeContextUsage | undefined> {
		return this._sendRequest('getContextUsage', { channel: session.toString() });
	}
