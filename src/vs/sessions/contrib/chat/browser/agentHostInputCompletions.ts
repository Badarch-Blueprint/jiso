...
export class AgentHostInputCompletionHandler extends AgentHostInputCompletionsBase<void, string> {
	...
	protected override _buildItem(position: Position, item: IChatInputCompletionItem): CompletionItem {
		...
		switch (attachment.kind) {
			...
			default: {
				...
				// File/directory mentions are surfaced as a chip in the attach
				// row (NewChatContextAttachments) above the input rather than as
				// inline `@path` text. Accepting the completion removes the typed
				// `@query` (replace range -> empty insert) and adds the resource
				// as a structured attachment. Claude still receives the file:
				// `resolvePromptForCli` rebuilds an `@<path>` "Attached
				// references" block from the attachment when sending.
				return {
	...
	acceptCompletion(entry: IChatRequestVariableEntry, insertText: string, range: OffsetRange | undefined): void {
		// Only track an inserted reference when the completion actually wrote
		// inline text (commands/skills). Chip-only mentions (files/directories)
		// pass empty text and are tracked solely as attachments, so they render
		// as a chip in the attach row instead of a highlighted inline `@path`.
		if (insertText) {
