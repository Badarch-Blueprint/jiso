...
import { isLocalClaudeProvider } from '../../../../platform/agentHost/common/agentService.js';
...
export class ModelPicker extends Disposable {
	...
	constructor(
		...
		this._delegate = {
			...
			// FORK: agent-host sessions — show a loading state while the host's
			// first auth/model pass settles, and never offer the Copilot
			// sign-in fallback for local-Claude sessions (they run on the
			// user's own Claude login; Copilot must not be reachable there).
			isLoadingModels: () => this._isAgentHostModelsPending(),
			copilotSetupApplies: () => this._copilotSetupApplies(),
		...
		this._register(autorun(reader => {
			...
			// FORK: re-run when the agent host's first auth/model pass settles so
			// the picker leaves its "Loading Models…" state even if no model-list
			// change event fires (e.g. the host reports zero models).
			if (provider && isAgentHostProvider(provider)) {
				provider.authenticationPending.read(reader);
			}
		...
		try {
			if (session && !isNewSession) {
				...
				if (!sessionModelId || sessionModel || !this._hasResolvedSessionModelVendor(sessionModelId)) {
					// FORK: sessions don't control the picker — a session sometimes echoes
					// a model id outside the fetched list (an SDK-resolved alias, a removed
					// model, a restore race). When the session's id can't be matched to a
					// listed model, keep a still-valid current selection instead of
					// blanking it; only a resolvable session model may replace it.
					const keepCurrent = !sessionModel && current && models.some(m => m.identifier === current.identifier);
					if (!keepCurrent) {
	...
	/**
	 * FORK: true while the active session's agent-host provider is still
	 * settling its first authentication/model pass — the window in which the
	 * model list is empty because it has not ARRIVED yet, not because there
	 * are no models. The picker shows "Loading Models…" instead of the empty
	 * or Copilot sign-in states during it.
	 */
	private _isAgentHostModelsPending(): boolean {
		const session = this._session.get();
		const provider = session ? this._sessionsProvidersService.getProvider(session.providerId) : undefined;
		return !!provider && isAgentHostProvider(provider) && provider.authenticationPending.get();
	}

	/**
	 * FORK: the Copilot sign-in / setup fallback never applies to local-Claude
	 * sessions (`ISession.sessionType` is the agent provider name — see
	 * `resourceSchemeForProvider`); they run on the user's own Claude login.
	 */
	private _copilotSetupApplies(): boolean {
		const session = this._session.get();
		const provider = session ? this._sessionsProvidersService.getProvider(session.providerId) : undefined;
		if (!session || !provider || !isAgentHostProvider(provider)) {
			return true;
		}
		return !isLocalClaudeProvider(session.sessionType);
	}
