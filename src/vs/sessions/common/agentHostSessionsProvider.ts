...
export interface IAgentHostSessionsProvider extends ISessionsProvider {
	/**
	 * FORK: whether the host's first authentication/model pass is still
	 * settling (sticky — once false it stays false). While true, the host's
	 * model lists may not have arrived yet; UI (e.g. the model picker) shows
	 * a loading state instead of an empty / sign-in fallback.
	 */
