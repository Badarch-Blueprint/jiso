...
import { IAgentHostService, isAgentHostEnabled } from '../../platform/agentHost/common/agentService.js';
...
class SessionsSetUpWidget extends Disposable {
	...
	private _start(): void {
		...
		// FORK: the desktop gate decision needs the agent host's root config
		// (async — the host may still be connecting); see _startDesktop.
		void this._startDesktop();
	}

	/**
	 * FORK: JisoIDE runs Claude on the user's own Claude login by default
	 * (agent-host root config `claudeUseCopilotProxy` unset/false — mirrors
	 * `ClaudeAgent._resolveTransportMode`). In that mode chat and models need
	 * no GitHub account or Copilot entitlement, so the blocking GitHub
	 * sign-in welcome must not gate the sessions window. Only an explicit
	 * opt-in to the Copilot proxy re-enables the original gated flow. The
	 * chat layer is already decoupled the same way
	 * (`requiresCopilotSignIn: !isLocalClaudeProvider(...)` in
	 * agentHostChatContribution.ts).
	 */
	private async _startDesktop(): Promise<void> {
		// When the local agent host is disabled entirely, Claude cannot run and
		// the GitHub/Copilot flow is the only functional path — keep the
		// original gated behavior instead of consulting a host that will
		// never connect.
		if (!isAgentHostEnabled(this.configurationService)) {
			this._startGitHubGatedFlow();
			return;
		}
		// Race the root-config read against a timeout: a crashed or slow agent
		// host must not leave the window hanging with neither a welcome nor a
		// completed setup. On timeout, assume native (the product default) and
		// skip the gate — a GitHub sign-in dialog cannot fix a broken host.
		const usesCopilotProxy = await Promise.race([
			this._agentHostUsesCopilotProxy(),
			new Promise<boolean>(resolve => {
				this._register(disposableTimeout(() => {
					this.logService.warn('[sessions welcome] agent host root config not available within 10s — assuming native Claude transport');
					resolve(false);
				}, 10_000));
			}),
		]);
		...
		if (!usesCopilotProxy) {
			this.logService.info('[sessions welcome] agent host runs Claude natively — skipping the GitHub/Copilot sign-in gate');
			this.onCompleted();
			return;
		}
	...
	/** The original GitHub/Copilot-gated desktop welcome flow (proxy mode / agent host disabled). */
	private _startGitHubGatedFlow(): void {
	...
	/**
	 * Resolve the agent host's `claudeUseCopilotProxy` root config value,
	 * waiting for the first root-state snapshot when the host has not
	 * connected yet. Resolves `false` (native) when the key is unset —
	 * matching the agent host's own default — and when the subscription
	 * errored (a GitHub sign-in dialog cannot fix a broken agent host).
	 */
	private _agentHostUsesCopilotProxy(): Promise<boolean> {
		const read = (state: RootState | Error | undefined): boolean | undefined => {
			if (state === undefined) {
				return undefined;
			}
			...
			return state.config?.values?.[AgentHostConfigKey.ClaudeUseCopilotProxy] === true;
		};
		const current = read(this.agentHostService.rootState.value);
		if (current !== undefined) {
			return Promise.resolve(current);
		}
		return new Promise<boolean>(resolve => {
			// Registered on the widget so a dispose-before-connect tears the
			// listener down; disposed eagerly once the first snapshot decides.
			const listener = this._register(this.agentHostService.rootState.onDidChange(state => {
