...
import { Dimension, size, IDimension, getActiveDocument, getWindow, prepend, IDomPosition } from '../../base/browser/dom.js';
...
export abstract class Part<MementoType extends object = object> extends Component<MementoType> implements ISerializableView {
	...
	create(parent: HTMLElement, options?: object): void {
		...
		this.partLayout = new PartLayout(this.options, this.contentArea, () => this.titleArea, () => this.headerArea, () => this.footerArea);
...
class PartLayout {
	...
	constructor(
		...
		private readonly getTitleArea: () => HTMLElement | undefined = () => undefined,
		private readonly getHeaderArea: () => HTMLElement | undefined = () => undefined,
		private readonly getFooterArea: () => HTMLElement | undefined = () => undefined,
	) { }

	// FORK: an area that has been hidden via CSS (display:none) must not reserve its fixed height,
	// otherwise the content area is sized short and a blank gap appears (e.g. when the chat panel
	// hides its title/header in favor of the session tab strip).
	private isAreaVisible(area: HTMLElement | undefined): boolean {
		return !!area && getWindow(area).getComputedStyle(area).display !== 'none';
	}
	...
	layout(width: number, height: number): ILayoutContentResult {
		...
		if (this.options.hasTitle && this.isAreaVisible(this.getTitleArea())) {
		...
		if (this.headerVisible && this.isAreaVisible(this.getHeaderArea())) {
		...
		if (this.footerVisible && this.isAreaVisible(this.getFooterArea())) {
