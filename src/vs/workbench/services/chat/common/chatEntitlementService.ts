...
export class ChatEntitlementService extends Disposable implements IChatEntitlementService {
	...
	constructor(
		...
		if (!productService.extensionsGallery?.serviceUrl) {
			// FORK: without an extension gallery, chat setup can never succeed because it
			// has to install the default chat extension from the marketplace. Bail out so
			// no setup surfaces (setup agents, trigger actions, dialogs) are wired up and
			// chat runs solely on the built-in agent host (Claude) providers.
			return;
		}
