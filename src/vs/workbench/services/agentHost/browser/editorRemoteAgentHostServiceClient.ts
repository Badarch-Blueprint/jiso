...
import type { CompletionsParams, CompletionsResult, CreateTerminalParams, IClaudeContextUsage, IClaudePlanUsage, IClaudeSessionLogs, ResolveSessionConfigResult, SessionConfigCompletionsResult } from '../../../../platform/agentHost/common/state/protocol/commands.js';
...
export class EditorRemoteAgentHostServiceClient extends Disposable implements IAgentHostService {
	...
	getPlanUsage(): Promise<IClaudePlanUsage | undefined> {
		return this._requireClient().getPlanUsage();
	}

	getContextUsage(session: URI): Promise<IClaudeContextUsage | undefined> {
		return this._requireClient().getContextUsage(session);
	}

	getSessionLogs(session: URI): Promise<IClaudeSessionLogs | undefined> {
		return this._requireClient().getSessionLogs(session);
	}

	pingAgent(): Promise<boolean> {
		return this._requireClient().pingAgent();
	}
