...
export class ExtHostChatAgents2 extends Disposable implements ExtHostChatAgentsShape2 {
	...
	private async getModelForRequest(request: IChatAgentRequest, extension: IExtensionDescription): Promise<vscode.LanguageModelChat | undefined> {
		...
		// FORK: do not throw when no language model is registered. Participants that drive their
		// own backend (e.g. the Orchestra participant shelling out to Claude Code headless) never
		// read `request.model`, so an undefined model must not abort the request before the handler
		// runs. Participants that genuinely need a model still observe `request.model` as undefined.
		return model;
	}
