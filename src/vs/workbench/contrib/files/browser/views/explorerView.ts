...
import { IActiveWorkspaceFolderService } from '../../../../services/workspaces/common/activeWorkspaceFolderService.js';
...
export class ExplorerView extends ViewPane implements IExplorerView {
	...
	constructor(
		...
		@IActiveWorkspaceFolderService private readonly activeWorkspaceFolderService: IActiveWorkspaceFolderService
	) {
		...
		// Re-filter the tree to the active workspace folder when it changes (workspace switcher).
		this._register(this.activeWorkspaceFolderService.onDidChangeActiveFolder(() => this.setTreeInput()));
	}
	...
	get name(): string {
		// When the workspace switcher is filtering to a single active folder, show that folder's
		// name instead of the (possibly "Untitled") workspace label.
		const activeFolder = this.activeWorkspaceFolderService.activeFolder;
		if (activeFolder && this.contextService.getWorkbenchState() === WorkbenchState.WORKSPACE) {
	...
	protected override renderHeader(container: HTMLElement): void {
		...
		this._register(this.activeWorkspaceFolderService.onDidChangeActiveFolder(setHeader));
		setHeader();
	}
	...
	async setTreeInput(): Promise<void> {
		...
		if (this.contextService.getWorkbenchState() !== WorkbenchState.FOLDER || roots[0].error) {
			...
			// Workspace switcher: when an active folder is selected, show only that folder's
			// contents instead of every root.
			const activeFolder = this.activeWorkspaceFolderService.activeFolder;
			if (activeFolder) {
				const activeRoot = roots.find(root => this.uriIdentityService.extUri.isEqual(root.resource, activeFolder.uri));
				if (activeRoot) {
					input = activeRoot;
				}
			}
