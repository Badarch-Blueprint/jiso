...
class BrowserChatAgentToolsContribution extends Disposable implements IWorkbenchContribution {
	...
	private _updateBrowserContext(): void {
		...
		// FORK: publish browser context only when a page is actually SHARED.
		// Upstream also injected "No browser pages are currently shared…" /
		// "N pages are open but not shared…" whenever any browser tab merely
		// existed, which appended that boilerplate to every prompt at send
		// time (and into the provider transcript — see the replay mapper's
		// `stripInjectedContext`). With nothing shared there is nothing the
		// agent can act on, so say nothing.
		if (sharedViews.length === 0) {
			this.chatContextService.updateWorkspaceContextItems(BrowserChatAgentToolsContribution.CONTEXT_ID, []);
			return;
		}

		let value = 'The following browser pages are currently shared with you and can be interacted with using the browser tools:';
		value += '\n' + formatBrowserEditorList(this.editorService, sharedViews, { agentNetworkFilterService: this.agentNetworkFilterService });

		if (unsharedCount > 0) {
			value += `\n\n${unsharedCount} ${unsharedCount === 1 ? 'page is' : 'pages are'} open but not shared.`;
