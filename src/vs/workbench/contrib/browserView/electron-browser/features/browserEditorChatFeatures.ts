...
import { findElementMatches, pickSearchToken, selectAnchors, IElementNodeSignature, ISourceMatch, MIN_MATCH_SCORE, MATCH_AMBIGUITY_MARGIN } from '../../common/elementSourceMatcher.js';
...
export class BrowserEditorChatIntegration extends BrowserEditorContribution {
	...
	private async _attachElementDataToChat(elementData: IElementData, model: IBrowserViewModel) {
		...
		// Prefer a source reference: when the picked element can be confidently
		// mapped back to the markup that produced it, attach the real code as a
		// `file:lineRange` chip so the agent reads the source instead of a runtime
		// DOM snapshot. Fall back to the structured DOM context when no confident
		// source match is found (e.g. dynamically generated or third-party markup).
		const sourceLocation = await this._resolveElementSource(elementData, innerText ?? undefined);
		if (sourceLocation) {
			toAttach.push(toFileVariableEntry(sourceLocation.uri, sourceLocation.range));
		} else {
	...
	/**
	 * Best-effort map a picked DOM element back to the source markup that produced
	 * it. Searches workspace template files for the element (or, for a plain leaf,
	 * its nearest distinctive ancestor) and returns a file + line range only when a
	 * single confident, unambiguous match is found; otherwise returns undefined so
	 * the caller falls back to the DOM-context attachment.
	 */
	private async _resolveElementSource(elementData: IElementData, leafText: string | undefined): Promise<{ uri: URI; range: IRange } | undefined> {
		try {
			const ancestors = elementData.ancestors;
			if (!ancestors || ancestors.length === 0) {
				return undefined;
			}
			...
			const signature: IElementNodeSignature[] = ancestors.map(a => ({ tagName: a.tagName, id: a.id, classNames: a.classNames }));
			for (const anchor of selectAnchors(signature)) {
				const token = pickSearchToken(anchor);
				if (!token) {
					continue;
				}

				const files = await this._searchTemplateFiles(token, folders.map(f => f.uri));
				let best: { uri: URI; match: ISourceMatch } | undefined;
				let second: { uri: URI; match: ISourceMatch } | undefined;
				for (const uri of files) {
					...
					for (const match of findElementMatches(text, anchor, leafText)) {
						if (!best || match.score > best.match.score) {
							second = best;
							best = { uri, match };
						} else if (!second || match.score > second.match.score) {
							second = { uri, match };
						}
					}
				}

				if (best && best.match.score >= MIN_MATCH_SCORE && (!second || best.match.score - second.match.score >= MATCH_AMBIGUITY_MARGIN)) {
					this.logService.trace(`[BrowserEditor] Resolved element to source: ${best.uri.toString()}:${best.match.startLine}-${best.match.endLine} (score ${best.match.score})`);
					return { uri: best.uri, range: new Range(best.match.startLine, 1, best.match.endLine, 1) };
				}
			}
		} catch (error) {
			this.logService.trace('[BrowserEditor] Element source resolution failed', error);
		}
		return undefined;
	}

	/** Text-search workspace template files for a distinctive token, returning candidate files. */
	private async _searchTemplateFiles(token: string, folders: readonly URI[]): Promise<URI[]> {
		const query: ITextQuery = {
			type: QueryType.Text,
			contentPattern: { pattern: token, isWordMatch: true, isCaseSensitive: true },
			folderQueries: folders.map(folder => ({ folder })),
			includePattern: { '**/*.{html,htm,vue,svelte,jsx,tsx}': true },
			maxResults: 40,
		};
		try {
			const result = await this.searchService.textSearch(query, CancellationToken.None);
			return result.results.map(r => r.resource).slice(0, 30);
		} catch {
			return [];
		}
	}
