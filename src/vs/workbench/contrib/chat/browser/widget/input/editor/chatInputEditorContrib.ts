...
class InputEditorDecorations extends Disposable {
	...
	private async updateAsyncInputEditorDecorations(token: CancellationToken): Promise<void> {
		...
		for (const mentionRange of this.getPathLikeMentionRanges()) {
			varDecorations.push({ range: mentionRange });
		}

		this.widget.inputEditor.setDecorationsByType(decorationDescription, variableTextDecorationType, varDecorations);
	}

	private getPathLikeMentionRanges(): Range[] {
		...
		for (const match of text.matchAll(mentionRegex)) {
			...
			ranges.push(new Range(startPos.lineNumber, startPos.column, endPos.lineNumber, endPos.column));
		}
		return ranges;
	}
