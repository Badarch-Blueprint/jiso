...
export interface IModelPickerDelegate {
	 ...
	 * list (e.g. the local agent host is connecting / fetching models on
	 * startup). While this returns `true` and {@link getModels} is empty, the
	 * picker renders a transient "Loading Models…" state instead of the
	 * empty / sign-in fallbacks. Omitted => never loading.
	 */
	isLoadingModels?(): boolean;
	/**
	 * FORK: whether the Copilot sign-in / setup fallback applies to the active
	 * session. Local-Claude sessions return `false` — their models come from
	 * the user's own Claude login, so the picker must never offer a Copilot
