...
export function buildModelPickerItems(
	...
	if (loading && models.length === 0) {
		// FORK: the provider (e.g. the local agent host) is still fetching its
		// model list — a single disabled "Loading Models…" entry instead of the
		// empty / sign-in states below.
		items.push({
			item: {
				id: 'loadingModels',
				enabled: false,
				checked: false,
				class: undefined,
				tooltip: localize('chat.modelPicker.loadingModels', "Loading Models..."),
				label: localize('chat.modelPicker.loadingModels', "Loading Models..."),
				run: () => { }
			},
			kind: ActionListItemKind.Action,
			label: localize('chat.modelPicker.loadingModels', "Loading Models..."),
			group: { title: '', icon: ThemeIcon.fromId(Codicon.loading.id) },
			disabled: true,
			hideIcon: false,
		});
		return items;
...
export class ModelPickerWidget extends Disposable {
	...
	/**
	 * FORK: whether the active session's provider is still loading its model
	 * list with nothing to show yet (see {@link IModelPickerDelegate.isLoadingModels}).
	 */
	private _isLoadingModels(): boolean {
		return !!this._delegate.isLoadingModels?.() && this._delegate.getModels().length === 0;
	}

	private _requiresSetup(): boolean {
		// FORK: local-Claude sessions run on the user's own Claude login — the
		// Copilot sign-in / setup fallback never applies to them, regardless of
		// the Copilot entitlement state.
		if (this._delegate.copilotSetupApplies?.() === false) {
			return false;
		}
		// FORK: Copilot setup installs the default chat extension from the
		// marketplace — impossible without a configured gallery. Never surface
		// the "Sign in to use Copilot" state on such builds; the picker falls
		// through to the truthful loading / "No models available" states.
		if (!this._productService.defaultChatAgent || !this._productService.extensionsGallery?.serviceUrl) {
			return false;
		}
	...
	private _renderLabel(): void {
		...
		// FORK: the provider is still loading its model list (e.g. the local
		// agent host connecting / fetching on startup). Takes precedence over
		// the empty states below so startup never flashes "No models available".
		const loading = !unavailable && this._isLoadingModels();
		...
		const activating = !unavailable && !loading && this._activatingAfterTrust && this._delegate.getModels().length === 0;

		// Generic empty state (e.g. an agent-host session with no Auto fallback);
		// not evaluated while unavailable/loading/activating, which take precedence.
		const genericNoModels = !unavailable && !loading && !activating
			&& !(this._delegate.showAutoModel?.() ?? false)
			&& this._delegate.getModels().length === 0;
		const noModelsAvailable = unavailable || loading || activating || genericNoModels;
		...
		const modelLabel = unavailable
			...
			: loading
				? localize('chat.modelPicker.loadingModels', "Loading Models...")
				: activating
		...
		this._domNode.ariaLabel = restrictedMode
			...
			: setupRequired
				...
				: loading
					? localize('chat.modelPicker.ariaLabelLoading', "Models, loading")
