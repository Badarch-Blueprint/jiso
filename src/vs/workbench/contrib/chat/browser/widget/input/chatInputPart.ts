...
import { IChatModel, IChatModelInputState, IChatRequestModeInfo, IInputModel, logChangesToStateModel } from '../../../common/model/chatModel.js';
...
import type { IClaudeContextUsage } from '../../../../../../platform/agentHost/common/state/protocol/commands.js';
...
/** FORK: compact a token count, e.g. 1600 → "1.6k", 776 → "776". */
...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	// FORK: inline cumulative output-token readout (e.g. "↓ 1.6k tokens") shown next to the model picker.
	private _tokenUsageLabel?: HTMLElement;
	private readonly _tokenUsageResponseListener = this._register(new MutableDisposable<IDisposable>());
	/** FORK: live context-window usage (Claude Code's `↓ N tokens`), cached per completed request. */
	private _contextUsageTokens: number | undefined;
	private _contextUsageRequestId: string | undefined;
	...
	*/
	...
	private updateContextUsageWidget(): void {
		...
		// FORK: (re)subscribe the inline token-usage label to the current response's
		// streaming usage updates, in addition to the per-turn model changes below.
		const hookTokenUsageLabel = () => {
			const response = model.lastRequest?.response;
			this._tokenUsageResponseListener.value = response?.onDidChange(() => this._updateTokenUsageLabel(model));
			this._updateTokenUsageLabel(model);
			void this._refreshContextUsage(model);
		};
	...
	/**
	 * FORK: render the live context-window readout (e.g. "↓ 25k tokens") — Claude Code's `↓ N tokens`
	 * indicator, the precisely-tokenized current context (system prompt + tools + memory + history).
	 * The value is fetched from the live session via {@link _refreshContextUsage} after each turn and
	 * cached against the request id, so a stale cross-session value is never shown. Hidden until the
	 * first fetch resolves for the active session.
	 */
	private _updateTokenUsageLabel(model: IChatModel): void {
		const label = this._tokenUsageLabel;
		if (!label) {
			return;
		}
		const tokens = this._contextUsageRequestId === model.lastRequest?.id ? this._contextUsageTokens : undefined;
		if (typeof tokens !== 'number' || tokens <= 0) {
			label.style.display = 'none';
			label.textContent = '';
			return;
		}
		label.textContent = localize('chatTokenUsage', "↓ {0} tokens", formatTokenCount(tokens));
		label.style.display = '';
	}

	/**
	 * FORK: fetch the active session's live context-window usage (`Query.getContextUsage()`) once per
	 * completed turn and cache it against that turn's request id. No-op until the turn completes or
	 * when the provider can't report it (e.g. non-Claude sessions); errors degrade to "no readout".
	 */
	private async _refreshContextUsage(model: IChatModel): Promise<void> {
		const lastRequest = model.lastRequest;
		if (!lastRequest?.id || !lastRequest.response?.isComplete || this._contextUsageRequestId === lastRequest.id) {
			return;
		}
		const requestId = lastRequest.id;
		let usage: IClaudeContextUsage | undefined;
		try {
			usage = await this.agentHostService.getContextUsage(model.sessionResource);
		} catch {
			usage = undefined;
		}
		// The session may have moved on (switched / new turn) while awaiting; only apply if still current.
		if (model.lastRequest?.id !== requestId) {
			return;
		}
		this._contextUsageRequestId = requestId;
		this._contextUsageTokens = usage?.totalTokens;
		// FORK: feed the per-category breakdown (system prompt / tools / MCP / skills / messages) into
		// the context-usage details card so the hover shows where the loaded context is actually spent.
		this.contextUsageWidget?.setContextBreakdown(usage?.categories);
		this._updateTokenUsageLabel(model);
	}

	/**
	 * FORK: show the active session's harness-diagnostics log in a workbench
	 * dialog — the lines recorded by the agent host's client-side context
	 * management (tool-output trims, read dedupes, auto-compact triggers,
	 * per-call token usage, post-turn context-usage readings). Sessions from
	 * providers without diagnostics (or none recorded yet) get an explanatory
	 * notice.
	 */
	private _pingInFlight = false;
	private readonly _pingIconResetTimer = this._register(new MutableDisposable());

	/**
	 * FORK: run the agent host's 5-hour-window ping (one throwaway "hi" turn
	 * on the cheapest model, no session/transcript) and reflect the outcome on
	 * the wave icon: spinner while in flight, then check (started) or error
	 * (failed / proxy transport) for two seconds.
	 */
	private async _pingAgent(icon: HTMLElement): Promise<void> {
		if (this._pingInFlight) {
			return;
		}
		this._pingInFlight = true;
		this._pingIconResetTimer.clear();
		icon.classList.remove('codicon-pulse', 'codicon-check', 'codicon-error');
		icon.classList.add('codicon-loading', 'codicon-modifier-spin');
		let started = false;
		try {
			started = await this.agentHostService.pingAgent();
		} catch (err) {
			this.logService.warn('[Chat] pingAgent failed', err);
		}
		icon.classList.remove('codicon-loading', 'codicon-modifier-spin');
		icon.classList.add(started ? 'codicon-check' : 'codicon-error');
		this._pingIconResetTimer.value = disposableTimeout(() => {
			icon.classList.remove('codicon-check', 'codicon-error');
			icon.classList.add('codicon-pulse');
		}, 2000);
		this._pingInFlight = false;
	}

	private async _openSessionLogs(): Promise<void> {
		const sessionResource = this._widget?.viewModel?.model.sessionResource;
		if (!sessionResource) {
			return;
		}
		...
		try {
			lines = (await this.agentHostService.getSessionLogs(sessionResource))?.lines;
		} catch (err) {
			this.logService.warn('[Chat] getSessionLogs failed', err);
		}
		const markdown = new MarkdownString();
		if (lines && lines.length > 0) {
			// FORK: group the flat diagnostics into one section per prompt.
			// A `turn started` line (appended by `ClaudeAgentSession.send`)
			// opens a new section titled with the prompt's ordinal, wall-clock
			// time and snippet; everything before the first turn (materialize,
			// resume) renders under a leading "Session" section.
			interface ILogGroup { title: string; body: string[] }
			const groups: ILogGroup[] = [];
			let current: ILogGroup = { title: localize('chat.sessionLogs.sessionSection', "Session"), body: [] };
			let promptIndex = 0;
			for (const line of lines) {
				const turnMatch = /^(?<ts>\S+) turn started \(turnId [^)]+\)(?<rest>.*)$/.exec(line);
				if (!turnMatch) {
				...
				promptIndex++;
				const time = /T(?<hms>\d{2}:\d{2}:\d{2})/.exec(turnMatch.groups!.ts)?.groups?.hms;
				const snippet = / — prompt: (?<snippet>.*)$/.exec(turnMatch.groups!.rest)?.groups?.snippet;
				let title = localize('chat.sessionLogs.promptSection', "Prompt {0}", promptIndex);
				if (time) {
					title += ` · ${time}`;
				}
				if (snippet) {
					// Snippets land inside markdown bold — strip characters that
					// would break out of it (the session side already flattened
					// whitespace and backticks).
					title += ` — ${snippet.replace(/[\\*_[\]<>`]/g, '')}`;
				}
				current = { title, body: [line] };
			}
			groups.push(current);
			for (const group of groups) {
				markdown.appendMarkdown(`**${group.title}**\n\n`);
				markdown.appendCodeblock('text', group.body.join('\n'));
			}
		} else {
			markdown.appendText(localize('chat.sessionLogs.empty', "No harness diagnostics recorded for this session yet. Entries appear as turns run: tool-output trims, read dedupes, auto-compact triggers, token usage, and post-turn context-usage readings."));
		}
		// Export payload for Copy / Download — self-describing so a shared log
		// still identifies its session when it lands in a bug report.
		const exportText = `Harness diagnostics — ${sessionResource.toString()}\n\n${lines && lines.length > 0 ? lines.join('\n') : '(no entries)'}\n`;
		await this.dialogService.prompt({
			type: Severity.Info,
			message: localize('chat.sessionLogs.title', "Session Logs"),
			detail: sessionResource.toString(),
			buttons: [
				{
					label: localize('chat.sessionLogs.copy', "Copy"),
					run: () => this.clipboardService.writeText(exportText),
				},
				{
					label: localize('chat.sessionLogs.download', "Download"),
					run: async () => {
						const name = `claude-session-logs-${new Date().toISOString().replace(/[:.]/g, '-')}.log`;
						const target = await this.fileDialogService.showSaveDialog({
							title: localize('chat.sessionLogs.downloadTitle', "Save Session Logs"),
							defaultUri: joinPath(await this.fileDialogService.defaultFilePath(), name),
						});
						if (target) {
							await this.fileService.writeFile(target, VSBuffer.fromString(exportText));
						}
					},
				},
			],
			cancelButton: localize('chat.sessionLogs.close', "Close"),
			custom: {
				markdownDetails: [{ markdown, classes: ['chat-session-logs-content'] }],
			},
		});
	}
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
		...
		// FORK: harness action pills next to the context-usage gauge under the
		// input. Rendered as labeled chip buttons (icon + text, hover
		// background) so they read as real actions rather than footnotes.
		const buildHarnessPill = (iconClass: string, label: string, hover: string, onRun: (icon: HTMLElement) => void): { pill: HTMLElement; icon: HTMLElement } => {
			const pill = dom.append(this.contextUsageWidgetContainer, dom.$('div.chat-harness-action'));
			const icon = dom.append(pill, dom.$(`span.codicon.${iconClass}`));
			dom.append(pill, dom.$('span.chat-harness-action-label', undefined, label));
			pill.setAttribute('role', 'button');
			pill.tabIndex = 0;
			this._register(this.hoverService.setupDelayedHover(pill, { content: hover, appearance: { compact: true } }));
			this._register(dom.addDisposableListener(pill, dom.EventType.CLICK, () => onRun(icon)));
			this._register(dom.addDisposableListener(pill, dom.EventType.KEY_DOWN, e => {
		...
		// "Session Logs" — the harness-diagnostics log (context trimming / read
		// dedupe / auto-compact / token usage) for the active session.
		buildHarnessPill(
			'codicon-output',
			localize('chat.sessionLogs', "Logs"),
			localize('chat.sessionLogs.tooltip', "Show this session's harness diagnostics (context trimming, auto-compact, token usage)"),
			() => { void this._openSessionLogs(); },
		);

		// "Warm Up" — one minimal throwaway "hi" turn on the cheapest model so
		// the Claude subscription's 5-hour usage window starts ticking now.
		// Icon feedback: pulse → spinner while in flight → check/error → pulse.
		buildHarnessPill(
			'codicon-pulse',
			localize('chat.sessionPing', "Warm Up"),
			localize('chat.sessionPing.tooltip', "Start the 5-hour Claude session window now (sends a minimal \"hi\" turn)"),
			icon => { void this._pingAgent(icon); },
		);

		// FORK: the "↓ N tokens" footer readout is intentionally NOT created — it showed the
		// context-window GAUGE (current conversation size), which reads as a per-turn burn and is
		// misleading. Per-turn cost is reported by the "Billed: N" figure on each response instead
		// (see `chatListRenderer.ts`). `_tokenUsageLabel` stays undefined so `_updateTokenUsageLabel`
		// no-ops via its `if (!label) return` guard.
		...
		this.inputActionsToolbar = this._register(this.instantiationService.createInstance(MenuWorkbenchToolBar, this.options.renderInputToolbarBelowInput ? this.attachmentsContainer : toolbarsContainer, MenuId.ChatInput, {
			...
			actionViewItemProvider: (action, options) => {
				// FORK: hide the "Set Agent" (mode/agent) picker, the session-target picker, and the
				// continue-in (delegation) picker from the chat input. The fork drives a single local
				// Claude Code (CLI) agent, so these selectors are redundant noise.
				if (action.id === OpenModePickerAction.ID
		...
		this.secondaryToolbar = this._register(this.instantiationService.createInstance(MenuWorkbenchToolBar, this.secondaryToolbarContainer, MenuId.ChatInputSecondary, {
			...
			actionViewItemProvider: (action, options) => {
				// FORK: hide the session-target / continue-in (delegation) picker ("Claude Code (CLI)")
				// from the chat input footer — the fork uses a single local Claude Code (CLI) agent.
				if (action.id === OpenSessionTargetPickerAction.ID || action.id === OpenDelegationPickerAction.ID) {
					return new HiddenActionViewItem(action);
				}
