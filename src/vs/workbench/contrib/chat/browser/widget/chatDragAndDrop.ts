...
export class ChatDragAndDrop extends Themable {
	...
	private async drop(e: DragEvent): Promise<void> {
		...
		this.inlineClaudeCliPathMentions(contexts);
	}

	private inlineClaudeCliPathMentions(contexts: readonly IChatRequestVariableEntry[]): void {
		const widget = this.widgetRef();
		if (!widget?.lockedAgentId?.includes('claude-cli')) {
			return;
		}
		...
		const mentions = contexts.flatMap(context => {
			if ((context.kind === 'file' || context.kind === 'directory' || context.kind === 'implicit') && URI.isUri(context.value)) {
				const path = this.toRelativeClaudePath(context.value, widget.viewModel?.model.workingDirectory);
		...
		const missing = mentions.filter(m => !current.includes(m));
		if (missing.length === 0) {
			return;
		}
		const next = current.trim().length > 0 ? `${current}\n\n${missing.join('\n')}` : missing.join('\n');
		widget.inputEditor.setValue(next);
	}

	private toRelativeClaudePath(uri: URI, workingDirectory: URI | undefined): string {
		if (workingDirectory && workingDirectory.scheme === uri.scheme) {
			const rel = relativePath(workingDirectory, uri);
			if (rel) {
				const root = basename(workingDirectory);
				return root ? `${root}/${rel}` : rel;
			}
		}
		const workspaceFolder = this.workspaceContextService.getWorkspaceFolder(uri)?.uri;
		if (workspaceFolder && workspaceFolder.scheme === uri.scheme) {
			const rel = relativePath(workspaceFolder, uri);
			if (rel) {
				const root = basename(workspaceFolder);
				return root ? `${root}/${rel}` : rel;
			}
		}
