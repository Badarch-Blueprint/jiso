...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	private _layoutListForInputHeight(): void {
		...
		// FORK: the input height observable is driven by a ResizeObserver, so just after
		// the secondary toolbar/footer grows (mode, folder, token readout) it can lag the
		// real DOM height. Use the element's live offsetHeight as a floor so the list is
		// never laid out taller than the space the input actually occupies — otherwise the
		// last message ends up tucked behind the input. The small gap adds breathing room.
		const INPUT_LIST_GAP = 6;
		const observedInputHeight = this.inputPart.height.get();
		const liveInputHeight = this.inputPart.element?.offsetHeight || observedInputHeight;
		const inputHeight = Math.max(observedInputHeight, liveInputHeight) + INPUT_LIST_GAP;
