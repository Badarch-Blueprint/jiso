...
export class ChatListItemRenderer extends Disposable implements ITreeRenderer<ChatTreeItem, FuzzyScore, IChatListItemTemplate> {
	...
	private renderChatTreeItem(element: ChatTreeItem, index: number, templateData: IChatListItemTemplate): void {
		...
		// FORK: report the per-prompt OUTPUT token count — the exact number the `claude` CLI ticks up
		// as it works a prompt (the tokens the model generates across all tool-steps of one send),
		// so the IDE figure matches the CLI 1:1. This is deliberately NOT a billing-weighted cost
		// (which would add input/cache-read/cache-write at their rates and read far higher); users
		// compare this against the CLI's counter, not against list price.
		// `usage.completionTokens` is cumulative for the whole prompt (from the SDK `result` message).
		const usage = isResponseVM(element) ? element.model.usage : undefined;
		const outputTokens = usage && typeof usage.completionTokens === 'number' ? usage.completionTokens : undefined;
		if (outputTokens !== undefined && outputTokens > 0) {
			templateData.footerDetailsContainer.textContent = localize('chat.footer.billed', "Billed: {0}", outputTokens.toLocaleString());
		...
		// FORK: render assistant rows as content-only (no provider/avatar/name header chrome).
		const hideResponseHeaderChrome = isResponseVM(element);
		...
		templateData.username.classList.toggle('hidden', hideChatUserIdentity || hideResponseHeaderChrome);
		templateData.avatarContainer.classList.toggle('hidden', hideChatUserIdentity || hideResponseHeaderChrome);

		this.hoverHidden(templateData.requestHover);
		dom.clearNode(templateData.detail);
		if (isResponseVM(element) && !hideResponseHeaderChrome) {
		...
		const shouldShowHeader = (isResponseVM(element) && !this.rendererOptions.noHeader) && !isSystemInitiatedRequest && !hideResponseHeaderChrome;
