		...
		try {
			...
			if (content.icon) {
				if (ThemeIcon.isThemeIcon(content.icon)) {
					...
					// FORK: render image welcome icons (data:/file: URIs, e.g. the Claude logo) in
					// their own colors via background-image, rather than a mask recolored to the
					// theme icon foreground — so a brand logo keeps its real color.
