...
import { ChatContextUsageDetails, IChatContextUsageData, IChatContextUsageCategory } from './chatContextUsageDetails.js';
...
export class ChatContextUsageWidget extends Disposable {
	...
	/**
	 * FORK: the latest per-category context breakdown fetched out-of-band via `getContextUsage`
	 * (see {@link setContextBreakdown}). Held separately because it arrives async, independent of
	 * the per-response usage, and is merged into {@link currentData} on each render.
	 */
	private _contextBreakdown: readonly IChatContextUsageCategory[] | undefined;
	...
	/**
	 * FORK: supply the SDK's per-category context breakdown (fetched out-of-band via
	 * `getContextUsage`). Stored for subsequent renders and merged into the current data so an
	 * already-open details card updates immediately.
	 */
	setContextBreakdown(categories: readonly IChatContextUsageCategory[] | undefined): void {
		this._contextBreakdown = categories;
		if (this.currentData) {
			this.currentData = { ...this.currentData, contextBreakdown: categories };
			this._contextUsageDetails.value?.update(this.currentData);
		}
	}
	...
	private updateFromResponse(response: IChatResponseModel, modelId: string): void {
		...
		this.render({
			...
			contextBreakdown: this._contextBreakdown,
		});
		this.show();
	}
