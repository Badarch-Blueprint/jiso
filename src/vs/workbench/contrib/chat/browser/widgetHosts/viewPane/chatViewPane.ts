...
import { ChatSessionTabsControl } from './chatSessionTabsControl.js';
...
export class ChatViewPane extends ViewPane implements IViewWelcomeDelegate {
	...
	private registerListeners(): void {
		...
		// FORK: agent-host providers (e.g. the local Claude Code CLI) publish their chat session
		// contributions asynchronously, AFTER the view has already picked a default session at
		// startup. With no contribution visible yet, the default resolves to the built-in `local`
		// harness (which needs a Copilot model and triggers the sign-in gate). Re-select the
		// configured default once it becomes available, replacing a still-pristine local fallback.
		this._register(this.chatSessionsService.onDidChangeAvailability(() => this.onDidChangeSessionAvailability()));
	...
	/**
	 * FORK: when a non-local default provider (e.g. `claude-cli`) becomes available after startup,
	 * swap a pristine local fallback session for it so new chats use the configured default rather
	 * than the Copilot-requiring local harness. Only swaps a local session with no requests.
	 */
	private onDidChangeSessionAvailability(): void {
		if (this.restoringSession) {
			return;
		}
		const model = this._widget?.viewModel?.model;
		if (!model || model.hasRequests || getChatSessionType(model.sessionResource) !== localChatSessionType) {
			return; // no model yet (handled by onDidChangeAgents), or not a pristine local fallback
		}
		...
		if (defaultType === localChatSessionType) {
			return; // configured default is local, or its contribution is still not visible
		}
		this.restoringSession = this.acquireDefaultNewSession(CancellationToken.None).then(async modelRef => {
			if (!this._widget || !modelRef) {
				modelRef?.dispose();
				return;
			}
			const wasVisible = this._widget.visible;
			try {
				this._widget.setVisible(false);
				await this.showModel(CancellationToken.None, modelRef, true, true);
			} finally {
				this._widget.setVisible(wasVisible);
			}
		});
	...
	protected override renderBody(parent: HTMLElement): void {
		...
		// FORK: session tab bar (Cursor-style) at the very top of the chat panel.
		const tabsHost = append(parent, $('.chat-session-tabs-host'));
		this.sessionTabsHost = tabsHost;
		this.sessionTabsControl = this._register(this.instantiationService.createInstance(ChatSessionTabsControl, tabsHost, {
			getActiveSessionResource: () => this._widget?.viewModel?.sessionResource ?? this.modelRef.value?.object.sessionResource,
			openSession: (resource: URI) => { void this.loadSession(resource); },
		}));
	...
	private sessionTabsControl: ChatSessionTabsControl | undefined; // FORK: top-of-panel session tab strip
	private sessionTabsHost: HTMLElement | undefined; // FORK: the tab strip's host element, so its height can be reserved during layout
	...
	private async showModel(token: CancellationToken, modelRef?: IChatModelReference | undefined, startNewSession = true, ignoreTransferredSession = false): Promise<IChatModel | undefined> {
		...
		// FORK: keep the session tab strip's active highlight in sync.
		this.sessionTabsControl?.refresh();
	...
	private doLayoutBody(height: number, width: number): void {
		...
		// FORK: the session tab strip sits above everything else in the panel body
		// (a `flex:0 0 auto` sibling, ~35px when shown). Its height must be reserved
		// here or the chat widget is laid out that much too tall and the last message
		// ends up tucked behind the input.
		remainingHeight -= this.sessionTabsHost?.offsetHeight ?? 0;
