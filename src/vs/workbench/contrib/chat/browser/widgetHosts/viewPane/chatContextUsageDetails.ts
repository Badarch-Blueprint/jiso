...
/**
 * FORK: one row of the absolute per-category context breakdown reported by the agent host's
 * `getContextUsage` (system prompt, tools, MCP tools, skills, messages, …). Structurally matches
 * the protocol's `IClaudeContextUsageCategory` so it can be passed through unchanged.
 */
export interface IChatContextUsageCategory {
	name: string;
	tokens: number;
	isDeferred?: boolean;
}
...
export interface IChatContextUsageData {
	...
	sessionCost?: number;
	/**
	 * FORK: absolute per-category token counts from the SDK. When present, shown in the details card
	 * in place of the response's percentage-based `promptTokenDetails` (which Claude sessions lack).
	 */
	contextBreakdown?: readonly IChatContextUsageCategory[];
}
...
export class ChatContextUsageDetails extends Disposable {
	...
	update(data: IChatContextUsageData): void {
		...
		// Render token details breakdown if available. Prefer the SDK's absolute per-category counts
		// (Claude sessions), falling back to the response's percentage-based prompt details.
		if (data.contextBreakdown && data.contextBreakdown.length > 0) {
			this.renderContextBreakdown(data.contextBreakdown);
		} else {
	...
	/**
	 * FORK: render the SDK's absolute per-category context breakdown — sorted largest-first, with
	 * the raw token count per category — so the user can see exactly where the loaded context goes
	 * (system prompt, tools, MCP servers, skills, conversation). Reuses the same token-detail markup
	 * as {@link renderTokenDetails} for consistent styling.
	 */
	private renderContextBreakdown(categories: readonly IChatContextUsageCategory[]): void {
		dom.clearNode(this.tokenDetailsContainer);

		const visible = categories.filter(c => c.tokens > 0).slice().sort((a, b) => b.tokens - a.tokens);
		if (visible.length === 0) {
		...
		const section = this.tokenDetailsContainer.appendChild($('.token-category'));
		const header = section.appendChild($('.token-category-header'));
		header.textContent = localize('contextBreakdown', "Breakdown");

		for (const category of visible) {
			const itemRow = section.appendChild($('.token-detail-item'));
			const itemLabel = itemRow.appendChild($('.token-detail-label'));
			itemLabel.textContent = category.isDeferred
				? localize('deferredCategory', "{0} (on demand)", category.name)
				: category.name;
			const itemValue = itemRow.appendChild($('.token-detail-value'));
			itemValue.textContent = this.formatTokenCount(category.tokens, 1);
		}
	}
