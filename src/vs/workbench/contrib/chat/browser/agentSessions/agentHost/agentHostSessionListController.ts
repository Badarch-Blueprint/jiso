...
export class AgentHostSessionListController extends Disposable implements IChatSessionItemController {
	...
	private _makeItem(rawId: string, opts: {
		...
		// Provisional sessions have a synthetic `untitled-<uuid>` id; never leak that into the label
		// (it produced the nonsensical "Session untitled"). Show a stable default instead.
		const isUntitled = rawId.startsWith('untitled-');
		return {
			resource: this._resource(rawId),
			label: opts.title || (isUntitled ? localize('agentHost.newChatSession', "New Chat") : `Session ${rawId.substring(0, 8)}`),
			description,
