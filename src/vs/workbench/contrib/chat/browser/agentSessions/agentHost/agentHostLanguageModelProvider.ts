...
export class AgentHostLanguageModelProvider extends Disposable implements ILanguageModelChatProvider {
	...
	updateModels(models: readonly SessionModelInfo[]): void {
		// FORK: root state re-pushes the agent's models on EVERY root-state change
		// (session status churn, titles, new sessions, …). Only fire when the list
		// actually changed, so session activity cannot re-drive the model picker.
		if (equals(this._models, models)) {
			return;
		}
		this._models = models;
