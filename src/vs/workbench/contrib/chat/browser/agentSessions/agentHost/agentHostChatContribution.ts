...
/** FORK: provider id of the legacy hand-rolled local Claude Code CLI agent-host provider. */
const CLAUDE_CLI_PROVIDER_ID = 'claude-cli';

/** FORK: provider id of the native local Claude agent-host provider (SDK transport on the user's `claude` login). */
const CLAUDE_PROVIDER_ID = 'claude';

/**
 * FORK: both local Claude providers authenticate via the user's own `claude` login — neither needs
 * GitHub Copilot sign-in, and both get the Claude welcome + mark instead of the generic delegation UI.
 */
function isLocalClaudeProvider(provider: string): boolean {
	return provider === CLAUDE_PROVIDER_ID || provider === CLAUDE_CLI_PROVIDER_ID;
}
