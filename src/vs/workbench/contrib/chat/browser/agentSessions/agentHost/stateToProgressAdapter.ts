...
export function usageInfoToChatUsage(usage: UsageInfo | undefined): IChatUsage | undefined {
	const hasTokens = typeof usage?.inputTokens === 'number' || typeof usage?.outputTokens === 'number' || typeof usage?.cacheReadTokens === 'number';
	const copilotCredits = getCopilotCredits(usage);
	if (!hasTokens && copilotCredits === undefined) {
		return undefined;
	}
	// FORK: `promptTokens` is the full context occupancy (drives the context gauge): the new
	// uncached input + the reused cached context + anything newly written to cache this turn.
	// With Claude prompt caching the live `inputTokens` is tiny (only new, uncached tokens) while
	// the reused conversation lives in `cacheReadTokens`; counting only `inputTokens` would wildly
	// understate context. `cacheReadTokens` is surfaced separately so the footer can subtract the
	// cheap re-reads and show the genuinely-billed (new) token count. No-op for non-caching providers.
	const cacheReadTokens = usage?.cacheReadTokens ?? 0;
	const cacheCreationTokens = usage?.cacheCreationTokens ?? 0;
	return {
		kind: 'usage',
		promptTokens: (usage?.inputTokens ?? 0) + cacheReadTokens + cacheCreationTokens,
