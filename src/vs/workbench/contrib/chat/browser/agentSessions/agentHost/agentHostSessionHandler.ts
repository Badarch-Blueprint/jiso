...
import { AgentProvider, AgentSession, isLocalClaudeProvider, type IAgentConnection } from '../../../../../../platform/agentHost/common/agentService.js';
...
import {
	...
	type IBrowserViewVariableEntry,
...
export class AgentHostSessionHandler extends Disposable implements IChatSessionContentProvider {
	...
	private _registerAgent(): void {
		const agentData: IChatAgentData = {
			...
			// FORK: the local Claude agent is the chat panel's default agent. The chat
			// service hard-gates sendRequest on an activated `isDefault` agent for the
			// location; in builds without the Copilot chat extension (which contributes
			// the upstream default participant) nothing else provides one, and sending
			// fails with "No default agent". As a core agent this still yields to any
			// extension-contributed default (see `_preferExtensionAgent`), so dev
			// builds with the Copilot extension behave exactly as before.
			isDefault: isLocalClaudeProvider(this._config.provider),
			isDynamic: true,
			isCore: true,
	...
	private async _createAndSubscribe(sessionResource: URI, model: ModelSelection | undefined, fork?: { session: URI; turnIndex: number; turnId: string }, config?: Record<string, unknown>): Promise<URI> {
		...
		const additionalDirectories = this._resolveAdditionalDirectories(workingDirectory);
	...
	/**
	 * FORK: in a multi-root workspace, every workspace folder other than the
	 * session's working directory becomes an additional accessible directory, so
	 * one chat session can read/edit all repos at once (e.g. a backend + frontend).
	 * Returns `undefined` for single-root workspaces so the field is omitted.
	 */
	private _resolveAdditionalDirectories(workingDirectory: URI | undefined): URI[] | undefined {
		...
		const extras = folders
			.map(f => f.uri)
			.filter(uri => !workingDirectory || !isEqual(uri, workingDirectory));
	...
	private _convertVariableToAttachment(v: IChatRequestVariableEntry, sessionResource: URI, messageText?: string): MessageAttachment | MessageAttachment[] | undefined {
		...
		// File attachments: a Location → selection, a URI → resource.
		// FORK: `implicit` (the active/edited editor auto-injected as context) is intentionally NOT
		// converted — it fell through here as an ~1.6k `edited_text_file` payload on every turn, which
		// the terminal CLI never sends. We now send only EXPLICIT user attachments (drag-drop `file`,
		// pasted `paste` code blocks, browser-screenshot images, `symbol`/`directory`/`promptFile`);
		// the agent can still `Read` any file it needs, exactly like the CLI. Dropping `implicit` here
		// makes it match no branch below and return `undefined` (not attached).
		...
		if (isElementVariableEntry(v)) {
			return this._toSimpleAttachment(v.name, this._renderElementAttachment(v), v._meta, 'element', referenceRange);
		}
		if (isBrowserViewVariableEntry(v)) {
			return this._toSimpleAttachment(v.name, this._renderBrowserViewAttachment(v), v._meta, 'browserView', referenceRange);
		}
		return undefined;
	}
	...
	private _renderElementAttachment(v: IElementVariableEntry): string {
		const lines: string[] = [`Element reference: ${v.name}`];
		if (typeof v.innerText === 'string' && v.innerText.trim().length > 0) {
			lines.push(`Inner text: ${v.innerText.trim()}`);
		}
		if (v.attributes && Object.keys(v.attributes).length > 0) {
			lines.push(`Attributes: ${JSON.stringify(v.attributes)}`);
		}
		if (v.computedStyles && Object.keys(v.computedStyles).length > 0) {
			lines.push(`Computed styles: ${JSON.stringify(v.computedStyles)}`);
		}
		if (v.dimensions) {
			lines.push(`Bounds: top=${v.dimensions.top}, left=${v.dimensions.left}, width=${v.dimensions.width}, height=${v.dimensions.height}`);
		}
		if (v.ancestors && v.ancestors.length > 0) {
			const chain = v.ancestors.map(a => {
				const idPart = a.id ? `#${a.id}` : '';
				const classPart = a.classNames?.length ? `.${a.classNames.join('.')}` : '';
				return `${a.tagName}${idPart}${classPart}`;
			});
	...
	private _renderBrowserViewAttachment(v: IBrowserViewVariableEntry): string {
		return `Browser view reference: ${v.name}\nURI: ${v.value.toString()}\nBrowser ID: ${v.browserId}`;
	}
