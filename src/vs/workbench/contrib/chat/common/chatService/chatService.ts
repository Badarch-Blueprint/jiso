...
export interface IChatUsage {
	...
	/**
	 * Of {@link promptTokens}, how many were read from the prompt cache (reused
	 * context, billed at a reduced rate). Lets consumers separate genuinely-new
	 * tokens from cheap cache re-reads. Undefined for providers without caching.
	 */
	cacheReadTokens?: number;
	/**
	 * Of {@link promptTokens}, how many were written to the prompt cache this turn
	 * (the context being freshly cached — spikes when the cache TTL refreshes).
	 * Like {@link cacheReadTokens} this is context, not new user/assistant content,
	 * so consumers should group it with the cached total, not the headline count.
	 */
