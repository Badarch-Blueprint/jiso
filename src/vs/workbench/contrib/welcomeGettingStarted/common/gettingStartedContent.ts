...
// FORK: chat setup installs the default chat extension from the marketplace, which is
// impossible without a configured gallery — hide the Copilot setup steps in that case
// (chat runs on the built-in agent host providers instead).
const copilotSetupAvailable = !!(product.defaultChatAgent && product.extensionsGallery?.serviceUrl);
...
function createCopilotSetupStep(id: string, button: string, when: string, includeTerms: boolean): BuiltinGettingStartedStep {
	...
	return {
		...
		description,
		when: `${copilotSetupAvailable ? when : 'false'} && !chatSetupHidden && !chatSetupDisabledInWorkspace`,
		media: {
