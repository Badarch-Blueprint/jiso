...
export class StartupPageRunnerContribution extends Disposable implements IWorkbenchContribution {
	...
	private tryShowOnboarding(): void {
		...
		if (!this.productService.defaultChatAgent || !this.productService.extensionsGallery?.serviceUrl) {
			// FORK: the onboarding is a Copilot sign-in/setup flow that ends in a marketplace
			// install of the default chat extension — impossible without a configured gallery.
			// Chat runs on the built-in agent host (Claude) providers without any setup.
			return;
		}
